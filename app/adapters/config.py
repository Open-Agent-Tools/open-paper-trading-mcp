...
class AdapterFactoryConfig:
    ...
    adapter_types: Dict[str, str] = field(default_factory=lambda: {
    ...
    default_configs: Dict[str, Dict[str, Any]] = field(default_factory=lambda: {
        'test_data': {
            'enabled': True,
...
class AdapterFactory:
    ...
    def create_adapter(self, adapter_type: str, adapter_config: Optional[AdapterConfig] = None) -> Optional[QuoteAdapter]:
        """
        ...
        try:
            ...
            if adapter_type == 'test_data':
                ...
                adapter = adapter_class(current_date=current_date, config=expanded_config)
            else:
    ...
    def create_cached_adapter(self, adapter_type: str, 
                            adapter_config: Optional[AdapterConfig] = None,
                            cache: Optional[QuoteCache] = None) -> Optional[CachedQuoteAdapter]:
        """
    ...
    def configure_registry(self, registry: AdapterRegistry, 
                          enabled_adapters: Optional[List[str]] = None) -> None:
        """
        ...
        for adapter_type in enabled_adapters:
            ...
            try:
                if cache_enabled:
                    adapter = self.create_cached_adapter(adapter_type, cache=shared_cache)
                else:

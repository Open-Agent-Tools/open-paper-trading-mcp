...
class AccountCreate(BaseModel):
    ...
    owner: str = Field(..., min_length=2, max_length=50, description="Account owner name")
    starting_balance: float = Field(..., ge=100, le=1000000, description="Starting cash balance (min $100, max $1M)")
...
async def get_positions(
    ...
    try:
        ...
        positions_data.sort(key=lambda x: x["market_value"] or 0, reverse=True)
...
async def find_options_endpoint(
    ...
    try:
        service = get_trading_service()
        options_data = await service.find_tradable_options(symbol, expiration_date, option_type)
...
async def cancel_all_orders(
    ...
    try:
        ...
        if asset_type == "stock":
            ...
            result = {
                "cancelled_count": stock_result.get("cancelled_count", 0) + option_result.get("cancelled_count", 0),
                "stock_orders_cancelled": stock_result.get("cancelled_count", 0),
                "option_orders_cancelled": option_result.get("cancelled_count", 0),
            }
        else:
            raise ValueError(f"Invalid asset_type: {asset_type}. Must be 'stock', 'option', or None")
...
async def buy_stock(order_request: StockOrderRequest) -> dict[str, Any]:
    ...
    try:
        ...
        order = await service.place_stock_order(**order_data)
...
async def buy_stock_limit(order_request: StockLimitOrderRequest) -> dict[str, Any]:
    ...
    try:
        ...
        order = await service.place_stock_order(
            symbol=order_request.symbol,
            side="buy",
            quantity=order_request.quantity,
            order_type="limit",
...
async def buy_stock_stop_limit(order_request: StockStopLimitOrderRequest) -> dict[str, Any]:
    """
...
async def sell_stock_stop_limit(order_request: StockStopLimitOrderRequest) -> dict[str, Any]:
    """

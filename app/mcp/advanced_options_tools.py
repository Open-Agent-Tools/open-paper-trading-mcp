...
async def calculate_option_chain_greeks(symbol: str, expiration_date: str) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for strike in strikes:
            ...
            put_theta = call_theta - risk_free_rate * strike * math.exp(-risk_free_rate * time_to_exp)
        ...
        data = {
            ...
            "portfolio_greeks": {
                "total_delta": sum(opt["call_greeks"]["delta"] for opt in option_greeks),
                "total_gamma": sum(opt["call_greeks"]["gamma"] for opt in option_greeks),
                "total_theta": sum(opt["call_greeks"]["theta"] for opt in option_greeks),
...
async def analyze_volatility_skew(symbol: str, expiration_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        atm_iv = next((item["call_iv"] for item in skew_data if abs(item["moneyness"] - 1) < 0.05), base_iv)
        otm_put_iv = next((item["put_iv"] for item in skew_data if item["moneyness"] < 0.9), base_iv)
        otm_call_iv = next((item["call_iv"] for item in skew_data if item["moneyness"] > 1.1), base_iv)
        ...
        data = {
            ...
            "skew_metrics": {
                ...
                "skew_type": "put_skew" if skew_slope > 0.1 else "call_skew" if skew_slope < -0.1 else "normal"
            },
            "interpretation": {
                "market_sentiment": "bearish" if skew_slope > 0.2 else "bullish" if skew_slope < -0.2 else "neutral",
                "volatility_environment": "high" if atm_iv > 0.30 else "low" if atm_iv < 0.15 else "normal"
            },
            "timestamp": datetime.now().isoformat()
        }
...
async def calculate_max_pain(symbol: str, expiration_date: str) -> dict[str, Any]:
    ...
    try:
        ...
        for strike in strikes:
            ...
            for other_strike in strikes:
                if other_strike < strike:  # ITM calls
                    call_pain = (strike - other_strike) * (1000 - abs(other_strike - 180) * 50)
                    total_call_pain += max(0, call_pain)
                
                if other_strike > strike:  # ITM puts
                    put_pain = (other_strike - strike) * (800 - abs(other_strike - 180) * 30)
        ...
        data = {
            ...
            "summary": {
                "total_call_oi": sum(item["call_open_interest"] for item in pain_analysis),
                "total_put_oi": sum(item["put_open_interest"] for item in pain_analysis),
                "put_call_oi_ratio": sum(item["put_open_interest"] for item in pain_analysis) / max(1, sum(item["call_open_interest"] for item in pain_analysis)),
                "max_pain_probability": 0.65 if abs(max_pain_point["strike"] - current_price) < 10 else 0.35
            },
            "interpretation": {
                "pressure_direction": "downward" if max_pain_point["strike"] < current_price else "upward" if max_pain_point["strike"] > current_price else "neutral",
                "significance": "high" if abs(max_pain_point["strike"] - current_price) > 5 else "moderate"
            },
            "timestamp": datetime.now().isoformat()
        }
...
async def get_put_call_ratio(symbol: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        data["interpretation"] = {
            ...
            "extremity": "extreme" if ratio > 1.5 or ratio < 0.5 else "moderate" if ratio > 1.1 or ratio < 0.8 else "normal",
...
async def get_unusual_options_activity(symbol: str = "", min_volume: int = 1000) -> dict[str, Any]:
    """
    ...
    try:
        ...
        unusual_activity = [
            {
                "symbol": "AAPL" if not symbol else symbol,
            ...
            {
                "symbol": "TSLA" if not symbol else symbol,
        ...
        if symbol:
            unusual_activity = [act for act in unusual_activity if act["symbol"] == symbol]
        
        # Filter by volume
        unusual_activity = [act for act in unusual_activity if act["volume"] >= min_volume]
        ...
        data = {
            ...
            "summary": {
                ...
                "avg_volume_spike": round(sum(act["volume_spike"] for act in unusual_activity) / max(1, len(unusual_activity)), 2),
                "call_put_split": {
                    "calls": len([act for act in unusual_activity if act["option_type"] == "call"]),
                    "puts": len([act for act in unusual_activity if act["option_type"] == "put"])
                }
            },

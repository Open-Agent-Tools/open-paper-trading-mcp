...
async def calculate_portfolio_beta() -> dict[str, Any]:
    ...
    try:
        ...
        for pos_beta in position_betas:
            pos_beta["weight"] = pos_beta["position_value"] / total_value if total_value > 0 else 0
...
async def calculate_sharpe_ratio(period_days: int = 252) -> dict[str, Any]:
    ...
    try:
        ...
        data = {
            ...
            "interpretation": {
                "rating": "excellent" if sharpe_ratio > 2.0 else "good" if sharpe_ratio > 1.0 else "fair" if sharpe_ratio > 0.5 else "poor",
                "description": f"Earning {sharpe_ratio:.2f} units of return per unit of risk"
            },
            "additional_metrics": {
                "treynor_ratio": round(treynor_ratio, 3),
                "information_ratio": information_ratio,
                "calmar_ratio": round(annual_return / 0.08, 3)  # Assuming 8% max drawdown
            },
...
async def calculate_var(confidence_level: float = 0.95, period_days: int = 1) -> dict[str, Any]:
    """
    ...
    try:
        ...
        total_value = sum(pos.quantity * pos.current_price for pos in portfolio.positions)
        ...
        data = {
            ...
            "interpretation": {
                ...
                "risk_level": "high" if var_percentage > 0.05 else "medium" if var_percentage > 0.02 else "low"
            },
            "expected_shortfall": {
...
async def get_portfolio_correlation() -> dict[str, Any]:
    ...
    try:
        ...
        for i, symbol1 in enumerate(symbols):
            ...
            for j, symbol2 in enumerate(symbols):
                if i == j:
                    ...
                    key = (symbol1, symbol2) if (symbol1, symbol2) in correlation_data else (symbol2, symbol1)
        ...
        avg_correlation = total_correlations / correlation_count if correlation_count > 0 else 0
        
        data = {
            "correlation_matrix": correlations,
            "average_correlation": round(avg_correlation, 3),
            "interpretation": {
                "diversification_level": "low" if avg_correlation > 0.7 else "medium" if avg_correlation > 0.4 else "high",
            ...
            "highest_correlation": max(
                [corr for row in correlations for corr in row["correlations"] if corr["correlation"] != 1.0],
...
async def analyze_sector_allocation() -> dict[str, Any]:
    ...
    try:
        ...
        for sector, data in sector_values.items():
            sector_percentage = (data["value"] / total_value * 100) if total_value > 0 else 0
        ...
        data = {
            ...
            "concentration_risk": {
                "top_sector_percentage": sector_allocations[0]["percentage"] if sector_allocations else 0,
                "risk_level": "high" if (sector_allocations[0]["percentage"] if sector_allocations else 0) > 40 else "medium" if (sector_allocations[0]["percentage"] if sector_allocations else 0) > 25 else "low"
            },
            "benchmark_comparison": benchmark_allocation,
            "diversification_score": min(100, max(0, 100 - (sector_allocations[0]["percentage"] if sector_allocations else 0))),
            "calculation_date": datetime.now().isoformat()
        }
...
async def get_risk_metrics() -> dict[str, Any]:
    ...
    try:
        ...
        risk_levels = {1: "very_low", 2: "low", 3: "medium", 4: "high", 5: "very_high", 6: "extreme"}
        ...
        data = {
            ...
            "recommendations": [
                "Consider diversifying across more sectors" if metrics["concentration_risk"]["largest_position_weight"] > 0.3 else "Sector diversification appears adequate",
                "Monitor volatility levels" if metrics["volatility"]["annual"] > 0.2 else "Volatility within acceptable range",
                "Review drawdown management" if metrics["downside_metrics"]["maximum_drawdown"] > 0.1 else "Drawdown control effective"
            ],
            "calculation_date": datetime.now().isoformat()
        }

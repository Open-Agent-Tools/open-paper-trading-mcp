...
from app.services.strategies import AdvancedStrategyAnalyzer, StrategyGreeks
...
class AccountLimits(BaseModel):
    ...
    max_position_size: float = Field(100000.0, description="Maximum position size in dollars")
    ...
    min_days_to_expiration: int = Field(1, description="Minimum days to expiration for new positions")
    max_strike_distance: float = Field(0.5, description="Maximum strike distance from ATM (as percentage)")
...
class AdvancedOrderValidator:
    ...
    def validate_order(self, 
                      ...
                      account_limits: Optional[AccountLimits] = None) -> ValidationResult:
        """
        ...
        if self._is_options_order(order):
            self._validate_options_requirements(order, current_quotes, account_limits, result)
        
        # Risk-based validation
        self._validate_risk_limits(account_data, order, current_quotes, account_limits, result)
        ...
        has_errors = any(msg.severity in [ValidationSeverity.ERROR, ValidationSeverity.CRITICAL] 
                        for msg in result.messages)
        result.is_valid = not has_errors
        result.can_execute = result.is_valid and result.risk_score < 90  # Don't execute very high risk
    ...
    def _validate_basic_requirements(self, 
        ...
        try:
            basic_errors = self.basic_validator.validate_order(account_data, order, None)
    ...
    def _validate_options_requirements(self, 
        ...
        if isinstance(order, MultiLegOrder):
            option_symbols = [leg.asset.symbol for leg in order.legs 
                            if isinstance(leg.asset, Option)]
        elif isinstance(order, Order) and isinstance(asset_factory(order.symbol), Option):
    ...
    def _validate_strike_price(self, 
        ...
        if strike_distance > account_limits.max_strike_distance:
            severity = ValidationSeverity.WARNING if strike_distance < 1.0 else ValidationSeverity.ERROR
            result.messages.append(ValidationMessage(
                rule=ValidationRule.OPTIONS,
                severity=severity,
                code="STRIKE_TOO_FAR",
        ...
        if strike_distance > 0.2:  # More than 20% from ATM
            moneyness = "ITM" if option.get_intrinsic_value(underlying_price) > 0 else "OTM"
            result.messages.append(ValidationMessage(
                rule=ValidationRule.OPTIONS,
    ...
    def _validate_assignment_risk(self, 
                                order: Union[Order, MultiLegOrder],
                                option: Option,
                                current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                result: ValidationResult) -> None:
        ...
        if isinstance(order, Order):
            ...
            for leg in order.legs:
                if (leg.asset.symbol == option.symbol and 
                    leg.order_type in [OrderType.SELL, OrderType.STO]):
                    is_short = True
                    break
        ...
        days_to_expiration = (option.expiration_date - date.today()).days
        if days_to_expiration <= 3 and intrinsic_value > -2.0:  # Close to expiration and near ITM
            result.messages.append(ValidationMessage(
                rule=ValidationRule.OPTIONS,
                severity=ValidationSeverity.WARNING,
    ...
    def _validate_risk_limits(self, 
        ...
        if self._is_options_order(order):
            self._validate_greeks_limits(account_data, order, current_quotes, account_limits, result)
        
        # Check concentration risk
        self._validate_concentration_risk(account_data, order, current_quotes, result)
    
    def _validate_greeks_limits(self, 
                              account_data: Dict[str, Any],
                              order: Union[Order, MultiLegOrder],
                              current_quotes: Dict[str, Union[Quote, OptionQuote]],
                              account_limits: AccountLimits,
        ...
        current_greeks = self.strategy_analyzer.aggregate_strategy_greeks(positions, current_quotes)
        
        # Estimate Greeks impact of new order (simplified)
        order_delta_impact = 0.0
        order_theta_impact = 0.0
        order_vega_impact = 0.0
    ...
    def _validate_compliance_rules(self, 
                                 account_data: Dict[str, Any],
                                 order: Union[Order, MultiLegOrder],
                                 account_limits: AccountLimits,
                                 result: ValidationResult) -> None:
    ...
    def _validate_strategy_rules(self, 
                               account_data: Dict[str, Any],
                               order: Union[Order, MultiLegOrder],
                               current_quotes: Dict[str, Union[Quote, OptionQuote]],
                               result: ValidationResult) -> None:
    ...
    def _validate_liquidity_requirements(self, 
                                       order: Union[Order, MultiLegOrder],
                                       current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                       result: ValidationResult) -> None:
        ...
        for symbol in symbols:
            ...
            if hasattr(quote, 'bid') and hasattr(quote, 'ask') and quote.bid and quote.ask:
                spread_percent = (quote.ask - quote.bid) / quote.price if quote.price > 0 else 0
    ...
    def _validate_option_liquidity(self, 
                                 symbol: str,
                                 current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                 result: ValidationResult) -> None:
    ...
    def _calculate_estimated_cost(self, 
                                order: Union[Order, MultiLegOrder],
                                current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> float:
    ...
    def _is_potential_day_trade(self, order: Union[Order, MultiLegOrder], account_data: Dict[str, Any]) -> bool:
    ...
    def _validate_multileg_strategy(self, 
                                  order: MultiLegOrder,
                                  current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                  result: ValidationResult) -> None:
    ...
    def _validate_position_conflicts(self, 
                                   account_data: Dict[str, Any],
                                   order: Union[Order, MultiLegOrder],
                                   result: ValidationResult) -> None:
        """Check for conflicting positions."""
    ...
    def _validate_concentration_risk(self, 
                                   account_data: Dict[str, Any],
                                   order: Union[Order, MultiLegOrder],
                                   current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                   result: ValidationResult) -> None:
        ...
        for position in positions:
            ...
            quote = current_quotes.get(symbol)
            if quote:
                position_value = abs(getattr(position, 'quantity', position.get('quantity', 0))) * quote.price
...
def validate_order_comprehensive(account_data: Dict[str, Any],
                               order: Union[Order, MultiLegOrder],
                               current_quotes: Dict[str, Union[Quote, OptionQuote]],
                               account_limits: Optional[AccountLimits] = None) -> ValidationResult:
    """Perform comprehensive order validation."""
...
def create_default_account_limits(options_level: int = 2, is_pdt: bool = False) -> AccountLimits:

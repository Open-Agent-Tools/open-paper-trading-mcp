...
from app.models.trading import Position, Order, OrderType
...
class ExpirationResult(BaseModel):
    ...
    expired_positions: List[Position] = Field(default_factory=list, description="Positions that expired")
    new_positions: List[Position] = Field(default_factory=list, description="New positions created from expiration")
    cash_impact: float = Field(0.0, description="Net cash impact from expiration")
    assignments: List[Dict[str, Any]] = Field(default_factory=list, description="Assignment details")
    exercises: List[Dict[str, Any]] = Field(default_factory=list, description="Exercise details")
    worthless_expirations: List[Dict[str, Any]] = Field(default_factory=list, description="Options expired worthless")
    warnings: List[str] = Field(default_factory=list, description="Warnings during processing")
    errors: List[str] = Field(default_factory=list, description="Errors during processing")
...
class OptionsExpirationEngine:
    ...
    def process_account_expirations(self, 
                                  account_data: Dict[str, Any], 
                                  quote_adapter: QuoteAdapter,
                                  processing_date: Optional[date] = None) -> ExpirationResult:
        """
        ...
        expired_positions = self._find_expired_positions(account['positions'], processing_date)
        ...
        for underlying_symbol, underlying_positions in expired_by_underlying.items():
            try:
                ...
                result.worthless_expirations.extend(underlying_result.worthless_expirations)
                result.warnings.extend(underlying_result.warnings)
                result.errors.extend(underlying_result.errors)
                
            except Exception as e:
                result.errors.append(f"Error processing {underlying_symbol} expirations: {str(e)}")
        
        # Remove expired positions with zero quantity
        account['positions'] = [pos for pos in account['positions'] 
    ...
    def _process_underlying_expirations(self, 
                                      account: Dict[str, Any], 
                                      underlying_symbol: str,
                                      expired_positions: List,
                                      quote_adapter: QuoteAdapter) -> ExpirationResult:
        ...
        try:
            ...
            result.errors.append(f"Error getting quote for {underlying_symbol}: {str(e)}")
            return result
        
        # Get current equity positions in this underlying
        equity_positions = self._get_equity_positions(account['positions'], underlying_symbol)
        long_equity = sum(pos['quantity'] for pos in equity_positions if pos['quantity'] > 0)
        short_equity = sum(pos['quantity'] for pos in equity_positions if pos['quantity'] < 0)
        ...
        for position in expired_positions:
            try:
                ...
                result.worthless_expirations.extend(position_result.worthless_expirations)
    ...
    def _process_single_expiration(self, 
                                 account: Dict[str, Any],
                                 position: Dict[str, Any],
                                 underlying_price: float,
                                 long_equity: int,
                                 short_equity: int) -> ExpirationResult:
        ...
        if not is_itm:
            ...
            position['quantity'] = 0
            result.expired_positions.append(Position(
                symbol=symbol,
                quantity=0,
    ...
    def _exercise_long_call(self, 
        ...
        self._add_position(account, call.underlying.symbol, new_quantity, new_cost_basis)
    ...
    def _assign_short_call(self, 
                         account: Dict[str, Any], 
                         call: Call, 
                         quantity: int,
                         underlying_price: float,
                         long_equity: int) -> Dict[str, Any]:
        ...
        if long_equity >= shares_needed:
            ...
            self._drain_asset(account['positions'], call.underlying.symbol, -shares_needed)
    ...
    def _assign_short_put(self, 
                        account: Dict[str, Any], 
                        put: Put, 
                        quantity: int,
                        underlying_price: float,
                        short_equity: int) -> Dict[str, Any]:
        ...
        if abs(short_equity) >= shares_to_buy:
            ...
            self._drain_asset(account['positions'], put.underlying.symbol, shares_to_buy)
            ...
            account['cash_balance'] -= cash_paid
            self._add_position(account, put.underlying.symbol, shares_to_buy, put.strike)
    ...
    def _drain_asset(self, positions: List, symbol: str, quantity: int) -> int:
        ...
        for pos in positions:
            ...
            if (pos_symbol == symbol and 
                copysign(1, pos_quantity) == copysign(1, quantity * -1)):
    ...
    def _add_position(self, account: Dict[str, Any], symbol: str, quantity: int, cost_basis: float):
            ...
            if isinstance(position, dict):
                ...
                if pos_symbol == symbol:
                    ...
                    if new_total_quantity != 0:
                        new_avg_cost = ((old_quantity * old_cost_basis) + (quantity * cost_basis)) / new_total_quantity

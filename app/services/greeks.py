...
def calculate_option_greeks(
    ...
    # Validate inputs
    if not _validate_inputs(option_type, strike, underlying_price, days_to_expiration, option_price):
        return greeks
...
def _validate_inputs(option_type: str, strike: float, underlying_price: float, 
                    days_to_expiration: int, option_price: float) -> bool:
...
def _black_scholes_call(S: float, K: float, r: float, q: float, T: float, sigma: float) -> float:
    """Black-Scholes call option price."""
    d1 = _d1(S, K, r, q, T, sigma)
    d2 = _d2(S, K, r, q, T, sigma)
    
    call_price = (S * math.exp(-q * T) * _normal_cdf(d1) - 
                  K * math.exp(-r * T) * _normal_cdf(d2))
    return call_price


def _black_scholes_put(S: float, K: float, r: float, q: float, T: float, sigma: float) -> float:
    """Black-Scholes put option price."""
    d1 = _d1(S, K, r, q, T, sigma)
    d2 = _d2(S, K, r, q, T, sigma)
    
    put_price = (K * math.exp(-r * T) * _normal_cdf(-d2) - 
                 S * math.exp(-q * T) * _normal_cdf(-d1))
    return put_price


def _implied_volatility_call(S: float, K: float, r: float, q: float, T: float, market_price: float) -> Optional[float]:
...
def _implied_volatility_put(S: float, K: float, r: float, q: float, T: float, market_price: float) -> Optional[float]:
...
def _implied_volatility_newton_raphson(option_type: str, S: float, K: float, r: float, q: float, 
                                     T: float, market_price: float, max_iterations: int = 100, 
...
def _calculate_all_greeks(option_type: str, S: float, K: float, r: float, q: float, 
                         T: float, sigma: float) -> Dict[str, float]:
    ...
    if option_type == 'call':
        greeks['charm'] = (q * math.exp(-q * T) * norm_d1 - 
                          math.exp(-q * T) * norm_pdf_d1 * (2 * (r - q) * T - d2 * sigma * sqrt_T) / 
                          (2 * T * sigma * sqrt_T)) / 365
    else:
        greeks['charm'] = (-q * math.exp(-q * T) * _normal_cdf(-d1) - 
                          math.exp(-q * T) * norm_pdf_d1 * (2 * (r - q) * T - d2 * sigma * sqrt_T) / 
                          (2 * T * sigma * sqrt_T)) / 365
    
    greeks['speed'] = -math.exp(-q * T) * norm_pdf_d1 * (d1 / (sigma * sqrt_T) + 1) / (S * S * sigma * sqrt_T)
    
    greeks['zomma'] = math.exp(-q * T) * norm_pdf_d1 * (d1 * d2 - 1) / (S * sigma * sigma * sqrt_T)
    
    greeks['color'] = (-math.exp(-q * T) * norm_pdf_d1 / (2 * S * T * sigma * sqrt_T) * 
                      (2 * q * T + 1 + (2 * (r - q) * T - d2 * sigma * sqrt_T) * d1 / (sigma * sqrt_T))) / 365
    
    greeks['veta'] = (S * math.exp(-q * T) * norm_pdf_d1 * sqrt_T * 
                     (q + ((r - q) * d1) / (sigma * sqrt_T) - (1 + d1 * d2) / (2 * T))) / (100 * 365)
    
    greeks['vomma'] = S * math.exp(-q * T) * norm_pdf_d1 * sqrt_T * d1 * d2 / sigma
    
    greeks['ultima'] = (-greeks['vomma'] / sigma * (d1 * d2 - d1 / d2 - d2 / d1 - 1)) / 365
...
def update_option_quote_with_greeks(option_quote, risk_free_rate: float = 0.02, 
                                   dividend_yield: float = 0.0):
    """Update an OptionQuote with calculated Greeks."""
    if (not option_quote.is_priceable() or 
        option_quote.underlying_price is None or 
        not hasattr(option_quote.asset, 'option_type')):
        return

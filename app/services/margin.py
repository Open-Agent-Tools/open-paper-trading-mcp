...
from .strategies import (
    BasicStrategy, AssetStrategy, SpreadStrategy, CoveredStrategy,
    StrategyType, SpreadType, group_into_basic_strategies
)
...
class MarginRequirement(BaseModel):
    ...
    details: Dict[str, Any] = Field(default_factory=dict, description="Calculation details")
...
class MarginCalculationResult(BaseModel):
    ...
    strategy_margins: List[MarginRequirement] = Field(default_factory=list, description="Per-strategy margins")
    calculation_timestamp: datetime = Field(default_factory=datetime.now, description="Calculation time")
...
class MaintenanceMarginService:
    ...
    def calculate_maintenance_margin(
        ...
        result = MarginCalculationResult(
            ...
            quote_source=getattr(adapter, '__class__', {}).get('__name__', 'unknown') if adapter else None
        )
    ...
    def _calculate_strategy_margin(self, strategy: BasicStrategy, quote_adapter) -> MarginRequirement:
    ...
    def _calculate_asset_margin(self, strategy: AssetStrategy, quote_adapter) -> MarginRequirement:
    ...
    def _calculate_short_stock_margin(self, strategy: AssetStrategy, quote_adapter) -> MarginRequirement:
        """Calculate margin for short stock position."""
        
        if quote_adapter is None:
            raise ValueError("Quote adapter required for short stock margin calculation")
    ...
    def _calculate_naked_option_margin(self, strategy: AssetStrategy, quote_adapter) -> MarginRequirement:
        ...
        option_premium = option_quote.price if option_quote and option_quote.price else 0.0
    ...
    def _calculate_naked_call_margin(self, call: Call, underlying_price: float, premium: float, quantity: int) -> float:
        """
    ...
    def _calculate_naked_put_margin(self, put: Put, underlying_price: float, premium: float, quantity: int) -> float:
        """
    ...
    def _calculate_spread_margin(self, strategy: SpreadStrategy, quote_adapter) -> MarginRequirement:
    ...
    def _calculate_covered_margin(self, strategy: CoveredStrategy, quote_adapter) -> MarginRequirement:
    ...
    def get_portfolio_margin_breakdown(self, positions: List[Position], quote_adapter=None) -> Dict[str, Any]:
        """Get detailed margin breakdown by underlying asset."""
        
        adapter = quote_adapter or self.quote_adapter
        result = self.calculate_maintenance_margin(positions=positions, quote_adapter=adapter)
        ...
        for margin_req in result.strategy_margins:
            ...
            if "asset" in margin_req.details:
                ...
                if len(asset_symbol) > 6:  # Likely an option symbol
                    underlying = asset_symbol.split('_')[0] if '_' in asset_symbol else asset_symbol[:4]
                else:
...
def get_margin_breakdown(positions: List[Position], quote_adapter=None) -> Dict[str, Any]:

...
from ..models.trading import MultiLegOrder, Order, OrderLeg, OrderType, OrderStatus, Position
...
class OrderExecutionResult:
    ...
    def __init__(self, success: bool, message: str = "", order_id: str = None,
                 cash_change: float = 0.0, positions_created: List[Position] = None,
...
class OrderExecutionEngine:
    ...
    async def execute_order(self, 
                          ...
                          estimator: Optional[PriceEstimator] = None) -> OrderExecutionResult:
        """
        ...
        try:
            ...
            total_order_price = sum(price * abs(leg.quantity) for leg, price in leg_prices.items())
            ...
            if current_cash + cash_requirement < 0:  # Negative cash_requirement means cash out
                return OrderExecutionResult(
                    success=False,
            ...
            for leg in order.legs:
                ...
                if leg.order_type in [OrderType.BTO, OrderType.STO]:
                    ...
                    # Closing position
                    affected_positions = self._close_position(leg, current_positions, cost_basis)
    ...
    async def execute_simple_order(self,
                                 account_id: str,
                                 order: Order,
                                 current_cash: float,
                                 current_positions: List[Position],
        ...
        return await self.execute_order(account_id, multi_leg, current_cash, current_positions, estimator)
    ...
    def _validate_order(self, order: MultiLegOrder):
        ...
        symbols = [leg.asset.symbol if hasattr(leg.asset, 'symbol') else str(leg.asset) for leg in order.legs]
        if len(symbols) != len(set(symbols)):
            raise OrderExecutionError("Duplicate assets not allowed in multi-leg orders")
    
    async def _calculate_leg_prices(self, legs: List[OrderLeg], estimator: PriceEstimator) -> Dict[OrderLeg, float]:
    ...
    def _validate_closing_positions(self, legs: List[OrderLeg], current_positions: List[Position]):
        ...
        for leg in legs:
            if leg.order_type in [OrderType.BTC, OrderType.STC]:
                ...
                closable_positions = [
                    ...
                    if pos.symbol == leg.asset.symbol and 
                    copysign(1, pos.quantity) == (copysign(1, leg.quantity) * -1)  # Opposite sign
                ]
                
                if not closable_positions:
                    raise OrderExecutionError(f"No available positions to close for {leg.asset.symbol}")
                
                # Check sufficient quantity
                available_quantity = sum(abs(pos.quantity) for pos in closable_positions)
    ...
    def _calculate_cash_requirement(self, legs: List[OrderLeg], leg_prices: Dict[OrderLeg, float]) -> float:
        ...
        for leg in legs:
            ...
            if leg.order_type.value.startswith('b') and (leg.quantity < 0 or cost_basis < 0):
                raise OrderExecutionError("Buy orders must have positive quantity and positive price")
            
            if leg.order_type.value.startswith('s') and (leg.quantity > 0 or cost_basis > 0):
                raise OrderExecutionError("Sell orders must have negative quantity and negative price")
            
            # Calculate cash impact with multiplier
            multiplier = 100 if isinstance(leg.asset, Option) else 1
            cash_impact = abs(cost_basis * leg.quantity) * copysign(1, leg.quantity) * multiplier
    ...
    def _close_position(self, leg: OrderLeg, current_positions: List[Position], 
                       fill_price: float) -> List[Position]:
        ...
        closable_positions = [
            pos for pos in current_positions 
            if pos.symbol == leg.asset.symbol and 
            copysign(1, pos.quantity) == (copysign(1, leg.quantity) * -1)
        ]
        ...
        for position in closable_positions:
            ...
            realized_pnl = (abs(fill_price) - position.avg_price) * quantity_to_close * position.multiplier
            if copysign(1, position.quantity + quantity_to_close) < 0:  # Was short position
                realized_pnl *= -1

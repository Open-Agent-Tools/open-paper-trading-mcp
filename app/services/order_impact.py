...
from ..models.trading import (
    Order, MultiLegOrder, OrderLeg, Position, OrderType, OrderStatus
)
...
class AccountSnapshot(BaseModel):
    ...
    maintenance_margin: float = Field(default=0.0, description="Required maintenance margin")
...
class OrderImpactAnalysis(BaseModel):
    ...
    estimated_fill_price: Optional[float] = Field(None, description="Estimated fill price")
    ...
    margin_impact: float = Field(..., description="Change in maintenance margin requirement")
    buying_power_impact: float = Field(..., description="Change in buying power")
    position_impact: Dict[str, Any] = Field(default_factory=dict, description="Changes to positions")
...
class OrderImpactService:
    ...
    def analyze_order_impact(
        ...
        after_snapshot = self._create_account_snapshot(after_account_data, quote_adapter)
        
        # Calculate impacts
        cash_impact = after_snapshot.cash_balance - before_snapshot.cash_balance
        margin_impact = after_snapshot.maintenance_margin - before_snapshot.maintenance_margin
        ...
        return OrderImpactAnalysis(
            ...
            estimated_fill_price=self._get_estimated_fill_price(order, estimated_fill_prices),
    ...
    def _create_account_snapshot(self, account_data: Dict[str, Any], quote_adapter=None) -> AccountSnapshot:
        ...
        total_invested = sum(
            pos.market_value for pos in position_objects 
    ...
    def _simulate_leg_execution(
        ...
        self._update_position_in_simulation(account_data, symbol, quantity, fill_price, asset)
        ...
        if existing_position is not None:
            ...
            if new_quantity == 0:
                ...
                if (old_quantity > 0 and quantity > 0) or (old_quantity < 0 and quantity < 0):
                    ...
                    total_cost = (old_quantity * old_avg_price) + (quantity * fill_price)
    ...
    def _assess_risks(
        ...
        if len(after.positions) > 0:
            largest_position_value = max(
                abs(pos.market_value) for pos in after.positions 
                if pos.market_value is not None
            )
            ...
            if isinstance(asset, Option):
                ...
                if hasattr(asset, 'get_days_to_expiration'):
                    ...
                    if days_to_exp is not None and days_to_exp < 7:
                        warnings.append(f"Short-dated option expiring in {days_to_exp} days")
                
                # Naked options warning
                if leg.order_type in [OrderType.STO] and not self._has_covering_position(asset, after.positions):
    ...
    def _has_covering_position(self, option: Option, positions: List[Position]) -> bool:
        ...
        for pos in positions:
            ...
            if (not pos.is_option and 
                pos.symbol == option.underlying.symbol and
                pos.quantity >= 100):  # Sufficient shares to cover
                return True
            
            # Check for covering option position
            if (pos.is_option and 
                hasattr(pos.asset, 'underlying') and
                pos.asset.underlying.symbol == option.underlying.symbol):
                # Add specific logic for different spread types
                return True
        
        return False
    
    def _get_order_type_description(self, order: Union[Order, MultiLegOrder, List[OrderLeg]]) -> str:
...
def analyze_order_impact(
    ...
    return service.analyze_order_impact(account_data, order, quote_adapter, estimated_fill_prices)

...
from app.models.trading import Order, MultiLegOrder, OrderLeg, OrderType, Position
from app.models.quotes import Quote, OptionQuote
from app.services.advanced_validation import (
    AdvancedOrderValidator, ValidationResult, AccountLimits
)
from app.services.strategies import (
    AdvancedStrategyAnalyzer, StrategyPnL, StrategyGreeks, StrategyRiskMetrics
)
from app.services.order_impact import OrderImpactService, OrderImpactAnalysis
from app.services.estimators import get_estimator_for_asset
...
class PriceScenario(BaseModel):
    ...
    underlying_price_change: float = Field(..., description="Underlying price change percentage")
    volatility_change: float = Field(0.0, description="Implied volatility change percentage")
...
class RiskMetrics(BaseModel):
    ...
    order_breakeven_moves: List[float] = Field(default_factory=list, description="Required moves to breakeven")
    ...
    # Time-based risks
    days_to_next_expiration: Optional[int] = Field(None, description="Days to nearest expiration")
    theta_decay_impact: float = Field(0.0, description="Expected theta decay to expiration")
...
class PreTradeAnalysis(BaseModel):
    ...
    # Core analysis
    validation_result: ValidationResult = Field(..., description="Order validation results")
    ...
    # Strategy analysis
    strategy_analysis: Dict[str, Any] = Field(default_factory=dict, description="Strategy-specific analysis")
    portfolio_greeks: StrategyGreeks = Field(..., description="Current portfolio Greeks")
    projected_greeks: StrategyGreeks = Field(..., description="Projected Greeks after order")
    
    # Scenario analysis
    scenario_results: List[ScenarioResult] = Field(default_factory=list, description="Stress test results")
    ...
    # Recommendations
    recommendations: List[Dict[str, Any]] = Field(default_factory=list, description="Risk management recommendations")
    alternative_strategies: List[Dict[str, Any]] = Field(default_factory=list, description="Alternative strategy suggestions")
    
    # Summary
    execution_recommendation: str = Field(..., description="Execute/modify/reject recommendation")
    confidence_level: float = Field(..., description="Analysis confidence level (0-100)")
    
    @property
    def should_execute(self) -> bool:
        """Whether order should be executed based on analysis."""
        return (self.validation_result.can_execute and 
                self.risk_metrics.overall_risk_level != RiskLevel.EXTREME and
                self.execution_recommendation in ["execute", "execute_with_caution"])
...
class PreTradeRiskAnalyzer:
    ...
    def analyze_order(self,
                     ...
                     include_scenarios: bool = True) -> PreTradeAnalysis:
        """
        ...
        risk_metrics = self._calculate_risk_metrics(
            account_data, order, current_quotes, validation_result,
            portfolio_greeks, projected_greeks
        )
        ...
        alternatives = self._suggest_alternatives(
            order, current_quotes, risk_metrics
        )
    ...
    def _calculate_projected_greeks(self,
                                  positions: List[Position],
                                  order: Union[Order, MultiLegOrder],
                                  current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> StrategyGreeks:
        ...
        if isinstance(order, Order):
            ...
            simulated_position = Position(
                ...
                avg_price=order.price or current_quotes.get(order.symbol, Quote(asset=asset, quote_date=datetime.now(), price=0)).price,
                current_price=current_quotes.get(order.symbol, Quote(asset=asset, quote_date=datetime.now(), price=0)).price,
                asset=asset
            )
            ...
            for leg in order.legs:
                simulated_position = Position(
                    ...
                    avg_price=leg.price or current_quotes.get(leg.asset.symbol, Quote(asset=leg.asset, quote_date=datetime.now(), price=0)).price,
                    current_price=current_quotes.get(leg.asset.symbol, Quote(asset=leg.asset, quote_date=datetime.now(), price=0)).price,
                    asset=leg.asset
                )
                simulated_positions.append(simulated_position)
    ...
    def _calculate_risk_metrics(self,
                              ...
                              projected_greeks: StrategyGreeks) -> RiskMetrics:
        ...
        concentration_risk = (order_value / portfolio_value * 100) if portfolio_value > 0 else 0
    ...
    def _analyze_order_strategy(self,
                              order: Union[Order, MultiLegOrder],
                              current_quotes: Dict[str, Union[Quote, OptionQuote]],
                              portfolio_greeks: StrategyGreeks) -> Dict[str, Any]:
        ...
        if isinstance(order, MultiLegOrder):
            analysis.update({
                ...
                "net_debit_credit": self._calculate_net_debit_credit(order, current_quotes),
    ...
    def _run_scenario_analysis(self,
                             account_data: Dict[str, Any],
                             order: Union[Order, MultiLegOrder],
                             current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> List[ScenarioResult]:
    ...
    def _generate_price_scenarios(self,
                                order: Union[Order, MultiLegOrder],
                                current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> List[PriceScenario]:
    ...
    def _analyze_scenario(self,
                        ...
                        order: Union[Order, MultiLegOrder],
                        current_quotes: Dict[str, Union[Quote, OptionQuote]],
                        scenario: PriceScenario) -> ScenarioResult:
    ...
    def _generate_recommendations(self,
                                ...
                                scenario_results: List[ScenarioResult]) -> List[Dict[str, Any]]:
        ...
        worst_scenario = min(scenario_results, key=lambda x: x.combined_pnl, default=None)
        if worst_scenario and worst_scenario.combined_pnl < -risk_metrics.portfolio_var_10day:
            recommendations.append({
                "type": "scenario_risk",
                "priority": "high",
    ...
    def _suggest_alternatives(self,
        ...
        if (isinstance(order, Order) and 
            isinstance(asset_factory(order.symbol), Option) and
            risk_metrics.overall_risk_level in [RiskLevel.HIGH, RiskLevel.EXTREME]):
    ...
    def _make_execution_recommendation(self,
                                     validation_result: ValidationResult,
                                     risk_metrics: RiskMetrics,
                                     scenario_results: List[ScenarioResult]) -> Tuple[str, float]:
    ...
    def _calculate_net_debit_credit(self, order: MultiLegOrder, 
                                  quotes: Dict[str, Union[Quote, OptionQuote]]) -> float:
    ...
    def _estimate_order_max_loss(self, order: Union[Order, MultiLegOrder],
                                quotes: Dict[str, Union[Quote, OptionQuote]]) -> float:
        ...
        if isinstance(order, Order):
            ...
            if quote:
                return abs(order.quantity) * quote.price * (100 if isinstance(asset_factory(order.symbol), Option) else 1)
        return 0.0
    
    def _calculate_portfolio_value(self, account_data: Dict[str, Any],
                                 quotes: Dict[str, Union[Quote, OptionQuote]]) -> float:
    ...
    def _find_nearest_expiration(self, order: Union[Order, MultiLegOrder]) -> Optional[int]:
    ...
    def _calculate_liquidity_risk(self, order: Union[Order, MultiLegOrder],
                                quotes: Dict[str, Union[Quote, OptionQuote]]) -> float:
        """Calculate liquidity risk score."""
        # Simplified liquidity risk calculation
        return 0.0
    
    def _apply_scenario_to_quotes(self, quotes: Dict[str, Union[Quote, OptionQuote]],
                                scenario: PriceScenario) -> Dict[str, Union[Quote, OptionQuote]]:
        """Apply scenario changes to quotes."""
        # Simplified scenario application
        return quotes.copy()
    
    def _calculate_portfolio_scenario_pnl(self, positions: List[Position],
                                        current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                        scenario_quotes: Dict[str, Union[Quote, OptionQuote]]) -> float:
        """Calculate portfolio P&L in scenario."""
        # Simplified P&L calculation
        return 0.0
    
    def _calculate_order_scenario_pnl(self, order: Union[Order, MultiLegOrder],
                                    current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                    scenario_quotes: Dict[str, Union[Quote, OptionQuote]]) -> float:
        """Calculate order P&L in scenario."""
        # Simplified P&L calculation
    ...
    def _calculate_scenario_greeks_impact(self, order: Union[Order, MultiLegOrder],
                                        current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                        scenario_quotes: Dict[str, Union[Quote, OptionQuote]]) -> StrategyGreeks:
...
def analyze_pre_trade_risk(account_data: Dict[str, Any],
                          order: Union[Order, MultiLegOrder],
                          current_quotes: Dict[str, Union[Quote, OptionQuote]],
                          account_limits: Optional[AccountLimits] = None) -> PreTradeAnalysis:
    """Perform comprehensive pre-trade risk analysis."""
    analyzer = PreTradeRiskAnalyzer()
    return analyzer.analyze_order(account_data, order, current_quotes, account_limits)


def quick_risk_check(order: Union[Order, MultiLegOrder],
                    current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> Dict[str, Any]:
    ...
    return {
        ...
        'key_warnings': [msg.message for msg in analysis.validation_result.warnings[:3]]
    }
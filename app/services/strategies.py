...
class StrategyGreeks(BaseModel):
    ...
    # Risk metrics
    delta_dollars: float = Field(0.0, description="Dollar delta (delta * underlying price)")
...
class StrategyRiskMetrics(BaseModel):
    ...
    expected_shortfall: Optional[float] = Field(None, description="Expected loss beyond VaR")
    
    # Time decay risk
    theta_risk_score: float = Field(0.0, description="Time decay risk (0-100 scale)")
    days_to_max_theta: Optional[int] = Field(None, description="Days to maximum theta decay")
...
class ComplexStrategy(BasicStrategy):
    ...
    strategy_type: Literal[StrategyType.SPREAD] = Field(default=StrategyType.SPREAD)
    complex_type: ComplexStrategyType = Field(..., description="Complex strategy subtype")
...
class AdvancedStrategyAnalyzer:
    ...
    def analyze_strategy_pnl(self, positions: List[Position], 
                           current_quotes: Dict[str, Union[Quote, OptionQuote]],
                           entry_date: Optional[date] = None) -> List[StrategyPnL]:
        """
        ...
        for strategy in strategies:
            pnl = self._calculate_strategy_pnl(strategy, positions, current_quotes, entry_date)
            strategy_pnls.append(pnl)
        
        return strategy_pnls
    
    def aggregate_strategy_greeks(self, positions: List[Position],
    ...
    def detect_complex_strategies(self, positions: List[Position]) -> List[ComplexStrategy]:
        """
        ...
        for underlying_symbol, underlying_positions in by_underlying.items():
            strategies = self._detect_complex_for_underlying(underlying_symbol, underlying_positions)
            complex_strategies.extend(strategies)
        
        return complex_strategies
    
    def calculate_risk_metrics(self, positions: List[Position],
                             historical_prices: Optional[List[Dict[str, float]]] = None) -> StrategyRiskMetrics:
        """
        ...
        if historical_prices:
            metrics = self._calculate_historical_risk_metrics(positions, historical_prices)
    ...
    def generate_optimization_recommendations(self, positions: List[Position],
                                           current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> List[Dict[str, Any]]:
        """
        ...
        strategy_recommendations = self._generate_strategy_specific_recommendations(positions, current_quotes)
        recommendations.extend(strategy_recommendations)
        
        return recommendations
    
    def _calculate_strategy_pnl(self, strategy: BasicStrategy, 
                              all_positions: List[Position],
                              current_quotes: Dict[str, Union[Quote, OptionQuote]],
                              entry_date: Optional[date]) -> StrategyPnL:
        ...
        pnl_percent = (total_pnl / total_cost_basis * 100) if total_cost_basis > 0 else 0.0
    ...
    def _get_strategy_positions(self, strategy: BasicStrategy, all_positions: List[Position]) -> List[Position]:
    ...
    def _get_strategy_name(self, strategy: BasicStrategy) -> str:
        ...
        if isinstance(strategy, AssetStrategy):
            ...
            return f"{strategy.option_type.title()} {strategy.spread_type.title()} Spread"
        elif isinstance(strategy, CoveredStrategy):
    ...
    def _detect_complex_for_underlying(self, underlying_symbol: str, 
                                     positions: List[Position]) -> List[ComplexStrategy]:
        ...
        straddles_strangles = self._detect_straddles_strangles(calls, puts, underlying_symbol)
        complex_strategies.extend(straddles_strangles)
        
        return complex_strategies
    
    def _detect_iron_condors(self, calls: List[Position], puts: List[Position], 
                           underlying_symbol: str) -> List[ComplexStrategy]:
        """Detect iron condor strategies."""
        # Iron condor requires 4 legs: short call, long call, short put, long put
        # Implementation would check for matching quantities and expirations
        # This is a simplified version
        return []
    
    def _detect_straddles_strangles(self, calls: List[Position], puts: List[Position],
                                  underlying_symbol: str) -> List[ComplexStrategy]:
        """Detect straddle and strangle strategies."""
        # Straddle: same strike calls and puts
        # Strangle: different strike calls and puts
        # Implementation would match positions by expiration and quantities
        return []
    
    def _calculate_historical_risk_metrics(self, positions: List[Position],
                                         historical_prices: List[Dict[str, float]]) -> StrategyRiskMetrics:
    ...
    def _calculate_theta_risk(self, options_positions: List[Position]) -> Dict[str, Any]:
        ...
        return {
            ...
            'days_to_max': int(min_days_to_expiry) if min_days_to_expiry != float('inf') else None,
            'total_theta': total_theta
        }
    
    def _calculate_assignment_risk(self, options_positions: List[Position]) -> Dict[str, Any]:
    ...
    def _generate_strategy_specific_recommendations(self, positions: List[Position],
                                                  current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> List[Dict[str, Any]]:
...
def analyze_advanced_strategy_pnl(positions: List[Position], 
                                current_quotes: Dict[str, Union[Quote, OptionQuote]],
                                entry_date: Optional[date] = None) -> List[StrategyPnL]:
...
def aggregate_portfolio_greeks(positions: List[Position],
                             current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> StrategyGreeks:
...
def get_portfolio_optimization_recommendations(positions: List[Position],
                                            current_quotes: Dict[str, Union[Quote, OptionQuote]]) -> List[Dict[str, Any]]:

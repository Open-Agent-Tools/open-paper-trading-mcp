...
class AccountValidator:
    ...
    def validate_account_state(self, 
                             ...
                             maintenance_margin: float = 0.0) -> bool:
        """
    ...
    def validate_order_pre_execution(self,
                                   order: MultiLegOrder,
                                   cash_balance: float,
                                   positions: List[Position],
    ...
    def _validate_leg(self, leg: OrderLeg, leg_name: str):
        ...
        if leg.order_type in [OrderType.BUY, OrderType.BTO, OrderType.BTC]:
            if leg.quantity < 0:
                raise ValidationError(f"{leg_name}: Buy orders must have positive quantity")
            if leg.price is not None and leg.price < 0:
                raise ValidationError(f"{leg_name}: Buy orders must have positive price")
        
        elif leg.order_type in [OrderType.SELL, OrderType.STO, OrderType.STC]:
            if leg.quantity > 0:
                raise ValidationError(f"{leg_name}: Sell orders must have negative quantity")
            if leg.price is not None and leg.price > 0:
                raise ValidationError(f"{leg_name}: Sell orders must have negative price")
    
    def _validate_closing_positions(self, legs: List[OrderLeg], positions: List[Position]):
        ...
        for leg in legs:
            if leg.order_type in [OrderType.BTC, OrderType.STC]:
                ...
                closable_positions = [
                    ...
                    self._positions_are_closable(pos.quantity, leg.quantity)
                ]
    ...
    def validate_position_limits(self,
                               ...
                               max_total_exposure: Optional[float] = None) -> bool:
        """
    ...
    def validate_risk_limits(self,
                           ...
                           max_portfolio_delta: Optional[float] = None) -> bool:
        """
    ...
    def _positions_are_closable(self, position_quantity: int, order_quantity: int) -> bool:
...
class OrderValidator:
    ...
    def _validate_multi_leg_strategy(self, order: MultiLegOrder):
        ...
        opening_legs = [leg for leg in order.legs if leg.order_type in [OrderType.BTO, OrderType.STO]]
        closing_legs = [leg for leg in order.legs if leg.order_type in [OrderType.BTC, OrderType.STC]]
...
def validate_account_state(cash_balance: float, 
                         positions: List[Position], 
                         maintenance_margin: float = 0.0) -> bool:
    """

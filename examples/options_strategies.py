...
from app.models.trading import MultiLegOrder, OrderLeg, OrderType, OrderSide, OrderCondition
from app.models.assets import Call, Put
...
class OptionsStrategyExamples:
    ...
    def covered_call_example(self) -> Dict[str, Any]:
        ...
        call_symbol = f"AAPL{expiration.strftime('%y%m%d')}C{int(strike_price * 1000):08d}"
    ...
    def cash_secured_put_example(self) -> Dict[str, Any]:
        ...
        put_symbol = f"AAPL{expiration.strftime('%y%m%d')}P{int(strike_price * 1000):08d}"
    ...
    def bull_call_spread_example(self) -> Dict[str, Any]:
        ...
        long_call_symbol = f"AAPL{expiration.strftime('%y%m%d')}C{int(long_strike * 1000):08d}"
        short_call_symbol = f"AAPL{expiration.strftime('%y%m%d')}C{int(short_strike * 1000):08d}"
    ...
    def iron_condor_example(self) -> Dict[str, Any]:
        ...
        order = MultiLegOrder(
            ...
            legs=[
                # Put spread legs
                OrderLeg(symbol=symbols["short_put"], side=OrderSide.SELL, quantity=Decimal("1"), price=short_put_premium),
                OrderLeg(symbol=symbols["long_put"], side=OrderSide.BUY, quantity=Decimal("1"), price=long_put_premium),
                # Call spread legs
                OrderLeg(symbol=symbols["short_call"], side=OrderSide.SELL, quantity=Decimal("1"), price=short_call_premium),
                OrderLeg(symbol=symbols["long_call"], side=OrderSide.BUY, quantity=Decimal("1"), price=long_call_premium)
            ],
        ...
        print(f"Underlying: {underlying_symbol} @ ${current_price}")
        print(f"Put Spread: Sell ${short_put_strike} put @ ${short_put_premium}, Buy ${long_put_strike} put @ ${long_put_premium}")
        print(f"Call Spread: Sell ${short_call_strike} call @ ${short_call_premium}, Buy ${long_call_strike} call @ ${long_call_premium}")
        print(f"Net Credit: ${net_credit}")
        
        # Calculate potential outcomes
        spread_width = short_put_strike - long_put_strike  # Both spreads have same width
        ...
        print(f"Maximum Profit: ${max_profit} (if ${short_put_strike} < stock < ${short_call_strike})")
        print(f"Maximum Loss: ${max_loss} (if stock <= ${long_put_strike} or >= ${long_call_strike})")
    ...
    def protective_put_example(self) -> Dict[str, Any]:
        ...
        print(f"Insurance Cost: {(put_premium / stock_price * 100):.1f}% of stock value")
...
def demonstrate_greeks_analysis():
    ...
    print(f"{'Scenario':<25} {'Delta':<8} {'Gamma':<8} {'Theta':<8} {'Vega':<8} {'IV':<8}")
    print("-" * 65)

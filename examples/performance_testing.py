...
from app.models.trading import Order, Position, OrderType, OrderSide, OrderCondition, OrderStatus
...
class PerformanceTester:
    ...
    async def time_async_function(self, func: Callable, *args, **kwargs) -> Dict[str, Any]:
    ...
    def run_benchmark(self, name: str, func: Callable, iterations: int = 100, *args, **kwargs) -> Dict[str, Any]:
    ...
    def print_benchmark_result(self, name: str, result: Dict[str, Any]):
        ...
        print(f"  Wall Time: {result['wall_time']['mean']:.4f}s ± {result['wall_time']['stdev']:.4f}s")
        print(f"  CPU Time: {result['cpu_time']['mean']:.4f}s ± {result['cpu_time']['stdev']:.4f}s")
        print(f"  Memory Peak: {result['memory']['mean_peak'] / 1024 / 1024:.2f} MB")
        print(f"  Range: {result['wall_time']['min']:.4f}s - {result['wall_time']['max']:.4f}s")
...
class GreeksPerformanceTesting:
    ...
    def test_bulk_greeks_calculation(self):
        ...
        return self.tester.run_benchmark(
            "Bulk Greeks Calculation (50 options)",
            calculate_bulk_greeks,
            iterations=20
        )
...
class OrderExecutionPerformanceTesting:
    ...
    def test_multi_leg_order_validation(self):
        ...
        def validate_multi_leg_order():
            ...
            legs = [
                OrderLeg(symbol="SPY240315P00420000", side=OrderSide.SELL, quantity=Decimal("1"), price=Decimal("2.50")),
                OrderLeg(symbol="SPY240315P00415000", side=OrderSide.BUY, quantity=Decimal("1"), price=Decimal("1.50")),
                OrderLeg(symbol="SPY240315C00470000", side=OrderSide.SELL, quantity=Decimal("1"), price=Decimal("2.20")),
                OrderLeg(symbol="SPY240315C00475000", side=OrderSide.BUY, quantity=Decimal("1"), price=Decimal("1.40"))
            ]
            ...
            validations = [
                ...
                sum(leg.quantity for leg in order.legs if leg.side == OrderSide.SELL)
            ]
...
class StrategyRecognitionPerformanceTesting:
    ...
    def test_simple_strategy_recognition(self):
        ...
        def recognize_simple_strategies():
            ...
            stock_positions = {pos.symbol: pos for pos in positions if not pos.symbol.endswith(('C', 'P'))}
            option_positions = {pos.symbol: pos for pos in positions if pos.symbol.endswith(('C', 'P'))}
            
            for stock_symbol, stock_pos in stock_positions.items():
                for opt_symbol, opt_pos in option_positions.items():
                    if (stock_symbol in opt_symbol and 
                        stock_pos.quantity > 0 and 
                        opt_pos.quantity < 0 and
                        opt_symbol.endswith('C')):  # Short call
                        strategies.append({
                            "type": "COVERED_CALL",
                            "stock": stock_symbol,
                            "option": opt_symbol
        ...
        return self.tester.run_benchmark(
            "Simple Strategy Recognition",
            recognize_simple_strategies,
            iterations=500
        )
    ...
    def test_complex_strategy_recognition(self):
        ...
        def recognize_complex_strategies():
            ...
            positions = [
                Position(account_id="TEST", symbol="SPY240315P00420000", quantity=Decimal("-1"), cost_basis=Decimal("2.50"), current_price=Decimal("2.30"), market_value=Decimal("-230.00")),
                Position(account_id="TEST", symbol="SPY240315P00415000", quantity=Decimal("1"), cost_basis=Decimal("1.50"), current_price=Decimal("1.40"), market_value=Decimal("140.00")),
                Position(account_id="TEST", symbol="SPY240315C00470000", quantity=Decimal("-1"), cost_basis=Decimal("2.20"), current_price=Decimal("2.00"), market_value=Decimal("-200.00")),
                Position(account_id="TEST", symbol="SPY240315C00475000", quantity=Decimal("1"), cost_basis=Decimal("1.40"), current_price=Decimal("1.30"), market_value=Decimal("130.00"))
            ]
            ...
            for pos in positions:
                ...
                if len(symbol) >= 15:  # Option symbol
                    underlying = symbol[:3] if symbol.startswith(('SPY', 'QQQ')) else symbol[:4]
                    expiration = symbol[3:9] if symbol.startswith(('SPY', 'QQQ')) else symbol[4:10]
            ...
            for key, group in by_underlying.items():
                if len(group) == 4:
                    ...
                    if len(puts) == 2 and len(calls) == 2:
                        ...
                        if (put_quantities.count(Decimal("1")) == 1 and 
                            put_quantities.count(Decimal("-1")) == 1 and
                            call_quantities.count(Decimal("1")) == 1 and 
                            call_quantities.count(Decimal("-1")) == 1):
                            strategies.append({
                                "type": "IRON_CONDOR",
                                "underlying": key,
                                "legs": len(group)
        ...
        return self.tester.run_benchmark(
            "Complex Strategy Recognition",
            recognize_complex_strategies,
            iterations=200
        )
...
def run_performance_test_suite():
    ...
    print(f"  Python Version: {psutil.Process().environ.get('PYTHON_VERSION', 'Unknown')}")
    ...
    results_summary["order_validation_simple"] = order_tester.test_simple_order_validation()
    results_summary["order_validation_multileg"] = order_tester.test_multi_leg_order_validation()
    ...
    results_summary["strategy_simple"] = strategy_tester.test_simple_strategy_recognition()
    results_summary["strategy_complex"] = strategy_tester.test_complex_strategy_recognition()
    ...
    for test_name, result in results_summary.items():
        if "error" not in result:
            ...
            print(f"{test_name:<35} {avg_time:<12.4f}s {success_rate:<12.1%} {memory_mb:<10.1f}MB")
        else:
    ...
    if "greeks_single" in results_summary and results_summary["greeks_single"]["wall_time"]["mean"] > 0.01:
        recommendations.append("Greeks calculation is slower than optimal. Consider caching or optimization.")
        
    if "greeks_bulk" in results_summary and results_summary["greeks_bulk"]["wall_time"]["mean"] > 2.0:
        recommendations.append("Bulk Greeks calculation needs optimization. Consider parallel processing.")
        
    memory_tests = ["memory_load", "cpu_intensive"]
    for test in memory_tests:
        if test in results_summary and results_summary[test]["memory"]["mean_peak"] > 100 * 1024 * 1024:  # > 100MB
            recommendations.append(f"High memory usage in {test}. Consider memory optimization.")

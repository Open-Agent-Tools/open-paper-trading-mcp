...
class PaperBroker:
    def __init__(
        ...
        account_adapter: AccountAdapter = None,
        market_adapter: MarketAdapter = None,
    ):
        self.quote_adapter = (
            quote_adapter if quote_adapter is not None else GoogleFinanceQuoteAdapter()
        )
        self.account_adapter = (
            account_adapter
            if account_adapter is not None
            else LocalFileSystemAccountAdapter()
        )
        self.market_adapter = (
            market_adapter
            if market_adapter is not None
            else PaperMarketAdapter(self.quote_adapter)
        )
    ...
    def get_option_quotes(self, underlying_asset=None, expiration_date=None):
        return self.get_options(
            underlying_asset=underlying_asset, expiration_date=expiration_date
        )
    ...
    def buy_to_open(
        self, account: Account = None, asset: Asset = None, quantity=1, simulate=False
    ):
        o = Order()
    ...
    def sell_to_open(
        ...
        o = Order()
        o.add_leg(
            asset=asset_factory(asset), quantity=abs(quantity) * -1, order_type="sto"
        )
        return self.enter_order(account, o, simulate=simulate)
    ...
    def sell_to_close(
        ...
        o = Order()
        o.add_leg(
            asset=asset_factory(asset), quantity=abs(quantity) * -1, order_type="stc"
        )
        return self.enter_order(account, o, simulate=simulate)
    ...
    def enter_order(
        ...
        if simulate:
            return self.market_adapter.simulate_order(
                account=account, order=order, estimator=estimator
            )

        # enter order always simulates first but only executes on simulate=False
        self.market_adapter.simulate_order(
            account=account, order=order, estimator=estimator
        )
        self.market_adapter.enter_order(
            account=account, order=order, estimator=estimator
        )
        self.account_adapter.put_account(account)
        return account
    ...
    def simulate_order(
        ...
        account_after = self.market_adapter.simulate_order(
            account=account, order=order, estimator=estimator
        )

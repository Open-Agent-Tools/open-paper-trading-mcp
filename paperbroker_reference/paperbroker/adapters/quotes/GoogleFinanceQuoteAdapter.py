...
class GoogleFinanceQuoteAdapter(QuoteAdapter):
    ...
    def get_quote(self, asset):
        ...
        if isinstance(asset, Option):
            ...
            if len(matches) == 0:
                raise Exception(
                    "GoogleFinanceAdapter.get_quote: No quote found for {}".format(
                        asset.symbol
                    )
                )
            return matches[0]
            ...
            return Quote(
                quote_date=arrow.now().format("YYYY-MM-DD"),
                asset=asset,
    ...
    def get_expiration_dates(self, underlying_asset=None):
        ...
        return sorted(
            list(
                set(
                    [
                        asset_factory(_["s"]).expiration_date
                        for _ in (oc.calls + oc.puts)
                    ]
                )
            )
        )

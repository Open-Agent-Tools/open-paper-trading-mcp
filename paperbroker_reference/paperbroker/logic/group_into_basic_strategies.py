...
class SpreadStrategy(BasicStrategy):
    ...
    def __init__(self, sell_option: Option, buy_option: Option, quantity=1):
        ...
        if sell_option.underlying != buy_option.underlying:
            raise Exception(
                "SpreadStrategy: underlying types of sell and buy must match"
            )
        ...
        if sell_option.option_type == "put":
            self.spread_type = (
                "credit"
                if self.sell_option.strike > self.buy_option.strike
                else "debit"
            )
        else:
            self.spread_type = (
...
class CoveredStrategy(BasicStrategy):
    ...
    def __init__(self, asset: Asset, sell_option: Option, quantity=1):
        ...
        if asset != sell_option.underlying:
            raise Exception(
                "CoveredStrategy: option underlying must be the same as asset"
            )
...
def _group_into_basic_strategies_in_underlying(underlying, positions):
    ...
    for short_call in short_calls:
        if long_equity.quantity >= 100:
            ...
            strategies.append(
                CoveredStrategy(asset=underlying, sell_option=short_call.asset)
            )
            ...
            strategies.append(
                SpreadStrategy(buy_option=long_call.asset, sell_option=short_call.asset)
            )
    ...
    for short_put in short_puts:
        if short_equity.quantity >= 100:
            ...
            strategies.append(
                CoveredStrategy(asset=underlying, sell_option=short_put.asset)
            )
            ...
            strategies.append(
                SpreadStrategy(buy_option=long_put.asset, sell_option=short_put.asset)
            )
...
def group_into_basic_strategies(positions):
    ...
    for underlying in underlyings:
        strategies += _group_into_basic_strategies_in_underlying(

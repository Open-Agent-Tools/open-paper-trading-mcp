...
def get_option_greeks(
    option_type, strike, underlying_price, days_to_expiration, price, dividend=0.0
):
    out = {
        "iv": None,
        "delta": None,
    ...
    if option_type is None or (option_type != "put" and option_type != "call"):
        return out
    if strike is None:
        return out
    ...
    sigma = (
        ...
        if option_type == "call"
        else ivolat3.ivolat_put(s, k, r, q, t, p)
    )
    ...
    out["delta"] = (
        ...
        if option_type == "call"
        else ivolat3.delta_put(s, k, r, q, t, sigma)
    )

    out["vega"] = ivolat3.vega(s, k, r, q, t, sigma)

    out["theta"] = (
        ivolat3.theta_call(s, k, r, q, t, sigma)
        if option_type == "call"
        else ivolat3.theta_put(s, k, r, q, t, sigma)
    ) / 365

    out["rho"] = (
        ivolat3.rho_call(s, k, r, q, t, sigma)
        if option_type == "call"
        else ivolat3.rho_put(s, k, r, q, t, sigma)
    )

    out["gamma"] = ivolat3.gamma(s, k, r, q, t, sigma)
    out["vanna"] = ivolat3.vanna(s, k, r, q, t, sigma)
    out["charm"] = (
        ivolat3.charm_call(s, k, r, q, t, sigma)
        if option_type == "call"
        else ivolat3.charm_put(s, k, r, q, t, sigma)
    ) / 365
    ...
    out["dual_delta"] = (
        ivolat3.dualdelta_call(s, k, r, q, t, sigma)
        if option_type == "call"
        else ivolat3.dualdelta_put(s, k, r, q, t, sigma)
    )

...
def get_maintenance_margin(
    strategies=None, positions=None, quote_adapter: QuoteAdapter = None
):
    ...
    for strategy in strategies:
        if strategy.strategy_type == "asset" and strategy.direction == "long":
            # no margin requirements for long anything
            total_margin_requirement += 0

        elif (
            strategy.strategy_type == "asset"
            ...
            total_margin_requirement += (
                abs(strategy.quantity) * quote_adapter.get_quote(strategy.asset).price
            )
        ...
        elif strategy.strategy_type == "spread" and strategy.spread_type == "debit":
            # no margin requirements for debit spreads
            total_margin_requirement += 0

        elif (
            strategy.strategy_type == "spread"
            and strategy.spread_type == "credit"
            and strategy.option_type == "put"
        ):
            # credit put spreads use the width of the strikes
            total_margin_requirement += (
                strategy.sell_option.strike - strategy.buy_option.strike
            ) * 100
            ...
            total_margin_requirement += (
                strategy.buy_option.strike - strategy.sell_option.strike
            ) * 100
            ...
            raise Exception(
                "A strategy was provided that we do not know how to calculate the maintenance margin for"
            )

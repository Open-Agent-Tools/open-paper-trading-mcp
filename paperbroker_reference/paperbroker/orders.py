...
class Leg(object):
    def __init__(
        self, asset: Asset, quantity: int, order_type: str, price: float = None
    ):
...
class Order(object):
    ...
    def buy_to_open(self, asset=None, quantity: int = None, price: float = None):
        return self.add_leg(
            asset=asset, quantity=quantity, price=price, order_type="bto"
        )

    def sell_to_open(self, asset=None, quantity: int = None, price: float = None):
        return self.add_leg(
            asset=asset, quantity=quantity, price=price, order_type="sto"
        )

    def buy_to_close(self, asset=None, quantity: int = None, price: float = None):
        return self.add_leg(
            asset=asset, quantity=quantity, price=price, order_type="btc"
        )

    def sell_to_close(self, asset=None, quantity: int = None, price: float = None):
        return self.add_leg(
            asset=asset, quantity=quantity, price=price, order_type="stc"
        )
    ...
    def add_leg(
        ...
        if leg is not None:
            if len([_.asset.symbol for _ in self.legs if _.asset == leg.asset]) > 0:
                raise Exception(
                    "Order.add_leg symbol {} already exists within this order".format(
                        leg.asset.symbol
                    )
                )
            self.legs.append(leg)
            self.price = (
                self.price + leg.price * abs(quantity)
                if self.price is not None and leg.price is not None
                else None
            )
        else:
            if len([_.asset.symbol for _ in self.legs if _.asset == asset]) > 0:
                raise Exception(
                    "Order.addLeg symbol {} already exists within this order".format(
                        asset.symbol
                    )
                )

            self.legs.append(
                Leg(asset=asset, quantity=quantity, order_type=order_type, price=price)
            )
            self.price = (
                self.price + price * abs(quantity)
                if self.price is not None and price is not None
                else None
            )

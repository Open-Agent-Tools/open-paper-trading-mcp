...
class TestDataQuoteAdapter(QuoteAdapter):
            ...
            for row in reader:
                if "AAL" in row[0]:
                    ...
                    if isinstance(asset, Option):
                        oq = OptionQuote(
                            quote_date=row[1], asset=row[0], bid=row[2], ask=row[3]
                        )
                        if oq.price:
                            underlying = oq.asset.underlying
                            greeks = get_option_greeks(
                                option_type=oq.asset.option_type,
                                underlying_price=testdata_keyvalue_cache.get(
                                    underlying.symbol
                                    + arrow.get(oq.quote_date).format("YYYY-MM-DD")
                                ).price,
                        ...
                        testdata_keyvalue_cache[row[0] + row[1]] = Quote(
                            quote_date=row[1], asset=row[0], bid=row[2], ask=row[3]
                        )
    ...
    def get_quote(self, asset):
        ...
        if (
            testdata_keyvalue_cache.get(
                asset.symbol + arrow.get(self.current_date).format("YYYY-MM-DD"), None
            )
            is not None
        ):
            return testdata_keyvalue_cache.get(
                asset.symbol + arrow.get(self.current_date).format("YYYY-MM-DD")
            )
    ...
    def get_options(self, underlying_asset=None, expiration_date=None):
        ...
        return [
            quote
            for quote in testdata_keyvalue_cache.values()
            if isinstance(quote, OptionQuote)
            and quote.quote_date == self.current_date
            and quote.asset.underlying
            == (
                underlying_asset
                if underlying_asset is not None
                else quote.asset.underlying
            )
            and quote.asset.expiration_date
            == (
                expiration_date
                if expiration_date is not None
                else quote.asset.expiration_date
            )
        ]
    ...
    def get_expiration_dates(self, underlying_asset=None):
        return sorted(
            list(
                set(
                    [
                        quote.asset.expiration_date
                        for quote in self.get_options(underlying_asset)
                    ]
                )
            )
        )

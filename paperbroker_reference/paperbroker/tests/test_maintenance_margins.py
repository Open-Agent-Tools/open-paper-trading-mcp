...
class TestMaintenanceMargins(unittest.TestCase):
    ...
    def test_get_maintenance_margin(self):
        ...
        positions = [
            ...
            Position(
                asset=Call(underlying="AAL", strike=5, expiration_date=arrow.now()),
                quantity=-2,
            ),
            Position(
                asset=Call(underlying="AAL", strike=10, expiration_date=arrow.now()),
                quantity=3,
            ),
            Position(
                asset=Call(underlying="AAL", strike=15, expiration_date=arrow.now()),
                quantity=-4,
            ),
            Position(
                asset=Call(underlying="AAL", strike=25, expiration_date=arrow.now()),
                quantity=2,
            ),
        ]
        ...
        assert (
            get_maintenance_margin(
                positions=positions, quote_adapter=self.quote_adapter
            )
            == 2500
        )

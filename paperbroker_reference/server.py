...
def get_options(asset=None, expiration_date=None, only_priceable=True):
    if only_priceable:
        return json(
            [_ for _ in broker.get_options(asset, expiration_date) if _.is_priceable()]
        )
    else:
...
def liquidate_account_positions(account_id: str, positions=None, simulate=None):
    simulate = (
        (not (not (request.args.get("simulate", False))))
    ...
    return json(
        broker.close_positions(
            account=account, positions=account.positions, simulate=simulate
        )
    )

...
class ImportChecker:
    ...
    def check_import(self, imp: dict, file_path: Path, relative_path: Path):
        ...
        if imp['type'] == 'import':
            ...
            if not self._module_exists(module_name):
                self.errors.append(f"{line_info}: Import error - module '{module_name}' not found")
            else:
                # Check for schema vs model confusion
                if 'schema' in module_name and 'model' in str(file_path):
                    self.warnings.append(f"{line_info}: Possible schema/model confusion - importing schema in model file")
                elif 'model' in module_name and 'schema' in str(file_path):
                    self.warnings.append(f"{line_info}: Possible model/schema confusion - importing model in schema file")
            ...
            if import_name != '*':
                if not self._name_exists_in_module(module_name, import_name):
                    self.errors.append(f"{line_info}: Import error - '{import_name}' not found in module '{module_name}'")
    ...
    def _check_schema_model_confusion(self, module_name: str, import_name: str, line_info: str):
        """Check for common schema/model confusion patterns."""
        # Pattern 1: Importing from schemas when it should be models
        if 'schemas' in module_name and import_name in ['Order', 'Position', 'Account', 'Transaction']:
            if 'database' not in module_name:  # Database models are OK
                self.warnings.append(f"{line_info}: Possible confusion - importing '{import_name}' from schemas (should it be from models?)")
        
        # Pattern 2: Importing Response models from wrong location
        if import_name.endswith('Response') and 'schemas' not in module_name:
            self.warnings.append(f"{line_info}: Response model '{import_name}' should typically be imported from schemas")
        
        # Pattern 3: SQLAlchemy models imported from wrong location
        if 'models.database' not in module_name and import_name in ['Base', 'Column', 'Integer', 'String']:

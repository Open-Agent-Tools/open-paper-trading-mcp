...
from app.models.database.trading import (  # noqa: E402
    Account,
    Order,
    Position,
    Transaction,
...
from app.schemas.orders import OrderStatus, OrderType  # noqa: E402
...
class UITesterProfileLoader:
    ...
    async def _create_xom_orders(self, db) -> int:
        ...
        for order_data in self.xom_orders:
            ...
            orders_created += 1
            print(f"  ✅ {order_data['action']} {order_data['quantity']} XOM @ ${order_data['price']:.2f} on {order_data['date']}")
    ...
    async def _create_transactions(self, db) -> int:
        ...
        for holding in self.stock_holdings:
            ...
            transaction_date = datetime.now() - timedelta(weeks=2, days=holding["quantity"] % 7)
        ...
        for order_data in self.xom_orders:
            transaction_id = str(uuid.uuid4())
            transaction_date = datetime.strptime(order_data["date"], "%Y-%m-%d").replace(
                hour=9, minute=35
            )
    ...
    async def verify_profile(self) -> dict:
        ...
        try:
            async for db in get_async_session():
                ...
                for holding in self.stock_holdings:
                    verification["portfolio_value"] += holding["quantity"] * holding["avg_cost"]
            
            # Print verification results
            print(f"  Account exists: {'✅' if verification['account_exists'] else '❌'}")

...
def check_test_content():
    ...
    try:
        ...
        expected_tools = [
            "cancel_order", "create_buy_order", "create_sell_order",
            "get_all_orders", "get_all_positions", "get_order",
            "get_portfolio", "get_portfolio_summary", "get_position",
            "get_stock_quote"
        ]
        
        eval_case = test_data["eval_cases"][0]
        final_response = eval_case["conversation"][0]["final_response"]["parts"][0]["text"]
...
def check_mcp_server():
    ...
    try:
        ...
        result = subprocess.run(
            ["uv", "run", "python", "-c", "from app.main import app; print('Server imports OK')"],
            capture_output=True,
            text=True,
            check=True
        )
...
def main():
    ...
    if all(checks):
        ...
        print("adk eval examples/google_adk_agent tests/evals/list_available_tools_test.json --config_file_path tests/evals/test_config.json")
        return True
    else:

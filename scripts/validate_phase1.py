...
class Phase1Validator:
    ...
    async def setup_test_database(self):
        ...
        test_account = DBAccount(
            id=str(uuid.uuid4()), owner="validation_test", cash_balance=100000.0
        )
        ...
        self.service.get_quote = MagicMock(
    ...
    async def validate_async_methods(self):
        ...
        assert portfolio.cash_balance == 100000.0, (
            f"Expected cash balance 100000.0, got {portfolio.cash_balance}"
        )
    ...
    async def validate_database_persistence(self):
        ...
        # Create an order
        order_data = OrderCreate(
            symbol="GOOGL", order_type=OrderType.BUY, quantity=50, price=2800.0
        )
        order = await self.service.create_order(order_data)
        order_id = order.id
        ...
        assert retrieved_order is not None, (
            "Order should persist across service instances"
        )
        assert retrieved_order.symbol == "GOOGL", (
            f"Expected symbol GOOGL, got {retrieved_order.symbol}"
        )
    ...
    async def validate_multi_leg_order_fix(self):
        ...
        assert db_order.account_id == self.test_account_id, (
            f"Expected account_id {self.test_account_id}, got {db_order.account_id}"
        )
    ...
    async def validate_account_state(self):
        ...
        assert isinstance(result, bool), (
            "Account state validation should return a boolean"
        )

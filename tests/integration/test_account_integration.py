...
class TestAccountIntegration:
    ...
    async def test_edge_cases_and_error_handling(self, db_session: AsyncSession):
        ...
        with patch('app.adapters.accounts.get_async_session') as mock_get_session:
            ...
            try:
                negative_balance_account = account_factory(cash=-1000.0)
                # If we get here, validation didn't work
                assert False, "Expected ValidationError for negative balance"
            except Exception:
    ...
    @pytest.mark.asyncio
    async def test_database_constraints_and_relationships(self, db_session: AsyncSession):
        ...
        with patch('app.adapters.accounts.get_async_session') as mock_get_session:
            ...
            for case in invalid_cases:
                try:
                    ...
                    assert False, f"Expected ValidationError for cash={case['cash']}"
                except Exception:
                    # Expected validation error - this is correct behavior
                    pass
    ...
    async def test_performance_with_many_accounts(self, db_session: AsyncSession):
        ...
        with patch('app.adapters.accounts.get_async_session') as mock_get_session:
            ...
            assert retrieval_time < 15.0  # Should retrieve 50 accounts in under 15 seconds

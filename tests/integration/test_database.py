...
from app.models.trading import OrderType, OrderStatus, OrderCondition, OrderSide
...
class TestAccountPersistence:
    ...
    async def test_account_balance_update(
        self, test_account: Account, db_session: AsyncSession
    ):
...
class TestOrderPersistence:
    ...
    async def test_simple_order_creation(
        self, test_account: Account, db_session: AsyncSession
    ):
...
class TestPositionManagement:
    ...
    async def test_option_position_with_greeks(
        self, test_account: Account, db_session: AsyncSession
    ):
    ...
    async def test_position_quantity_updates(
        self, test_account: Account, db_session: AsyncSession
    ):
...
class TestTransactionRecording:
    ...
    async def test_buy_transaction_recording(
        self, test_account: Account, db_session: AsyncSession
    ):
    ...
    async def test_option_transaction_recording(
        self, test_account: Account, db_session: AsyncSession
    ):
    ...
    async def test_transaction_history_queries(
        self, test_account: Account, db_session: AsyncSession
    ):
    ...
    async def test_option_chain_queries(self, db_session: AsyncSession):
        ...
        aapl_calls = await db_session.execute(
            select(OptionQuote).where(
                OptionQuote.underlying_symbol == "AAPL", OptionQuote.symbol.like("%C%")
            )
        )
...
class TestPerformanceQueries:
    ...
    async def test_bulk_transaction_insertion(
        self, test_account: Account, db_session: AsyncSession
    ):
    ...
    async def test_complex_portfolio_query(
        self, test_account: Account, db_session: AsyncSession
    ):
        ...
        portfolio_stats = await db_session.execute(
            select(
                ...
                func.sum(Position.quantity * Position.cost_basis).label(
                    "total_cost_basis"
                ),

...
from app.models.database.trading import Account as DBAccount, Position as DBPosition, Order as DBOrder
from app.schemas.orders import OrderType, OrderCondition, OrderCreate, OrderStatus
...
class TestDatabasePersistence:
    ...
    @pytest.mark.asyncio
    async def test_order_persistence_across_service_instances(self, db_session, test_account):
        ...
        service1.get_quote = MagicMock(return_value=StockQuote(
    ...
    @pytest.mark.asyncio
    async def test_portfolio_persistence_across_service_instances(self, db_session, test_account):
        ...
        service2 = TradingService(account_owner="test_user")
        service2._get_db_session = lambda: db_session
        service2.get_quote = MagicMock(return_value=StockQuote(
            symbol="AAPL", price=155.0, change=5.0, change_percent=3.33, volume=1000, last_updated=datetime.now()
        ))
    ...
    async def test_multi_leg_order_persistence(self, db_session, test_account):
        ...
        assert db_order.account_id == test_account.id  # Should use account.id, not account_owner

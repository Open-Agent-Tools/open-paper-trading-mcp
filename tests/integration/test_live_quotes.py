...
class TestLiveQuotes:
    ...
    async def test_account(self, trading_service):
        ...
        account = await trading_service.create_account(
            owner="test_user", initial_balance=10000.0
        )
        return account
    ...
    @pytest.mark.asyncio
    async def test_adapter_switching_test_to_robinhood(
        self, trading_service, sample_stock_quote
    ):
        ...
        with (
            patch("robin_stocks.robinhood.stocks.get_latest_price") as mock_price,
            patch(
    ...
    @pytest.mark.asyncio
    async def test_adapter_switching_robinhood_to_test(
        self, trading_service, sample_stock_quote
    ):
    ...
    async def test_quote_persistence_with_database(self, trading_service, test_account):
        ...
        async with get_async_session() as db:
            ...
            result = await db.execute(select(DBOrder).where(DBOrder.id == order.id))
    ...
    @pytest.mark.asyncio
    async def test_portfolio_calculation_with_live_quotes(
        self, trading_service, test_account
    ):
        """Test portfolio calculation using live quotes."""
        # Create some positions
        await trading_service.create_position(
            account_id=test_account.id, symbol="AAPL", quantity=100, avg_price=145.00
        )

        await trading_service.create_position(
            account_id=test_account.id, symbol="GOOGL", quantity=10, avg_price=2750.00
        )
        ...
        googl_position = next(
            pos for pos in portfolio.positions if pos.symbol == "GOOGL"
        )

        assert aapl_position.current_price is not None
        assert googl_position.current_price is not None
        assert (
            aapl_position.current_price != aapl_position.avg_price
        )  # Should be different from avg
    ...
    @pytest.mark.asyncio
    async def test_options_chain_integration(
    ...
    async def test_adapter_configuration_validation(self, trading_service):
        ...
        for adapter_type in valid_adapters:
            try:
                ...
                pytest.fail(
                    f"Valid adapter type {adapter_type} should not raise exception: {e}"
                )
    ...
    @pytest.mark.asyncio
    async def test_quote_data_consistency(
        self, trading_service, mock_robinhood_adapter
    ):

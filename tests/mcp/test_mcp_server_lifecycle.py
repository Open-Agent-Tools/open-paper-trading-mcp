...
class TestMCPServerLifecycle:
    ...
    def test_mcp_server_module_reload_safety(self):
        ...
        original_mcp = app.mcp.server.mcp
...
class TestMCPServerToolRegistration:
    ...
    def test_mcp_tool_registration_on_startup(self):
        ...
        assert registration_count > 15, f"Expected >15 tool registrations, found {registration_count}"
...
class TestMCPServerConfiguration:
    ...
    def test_mcp_server_architecture_pattern(self):
        ...
        unified_indicators = [
            'unified', 'combined', 'all MCP tools', 'Unified MCP server'
        ]
        
        has_unified_mention = any(indicator.lower() in source.lower() 
    ...
    def test_mcp_server_single_instance_pattern(self):
        ...
        assert fastmcp_count == 1, f"Should have exactly one FastMCP instance, found {fastmcp_count}"
    ...
    def test_mcp_server_architectural_notes(self):
        ...
        notes_found = sum(1 for note in architectural_notes 
                         if note.lower() in source.lower())
        assert notes_found >= 2, f"Should document architectural decisions, found {notes_found}"
...
class TestMCPServerConcurrency:
    ...
    def test_mcp_server_memory_efficiency(self):
        ...
        ref_count = sys.getrefcount(mcp)
        
        # Multiple imports shouldn't increase references significantly
        from app.mcp.server import mcp as mcp2
        new_ref_count = sys.getrefcount(mcp)

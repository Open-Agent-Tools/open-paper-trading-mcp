...
class TestPriority1Performance:
    ...
    def test_trading_service_initialization_performance(self):
        ...
        for _ in range(100):
            ...
            TradingService(account_owner=f"user_{_}")
        ...
        print(
            f"TradingService Init - Avg: {avg_time:.4f}s, Median: {median_time:.4f}s, Max: {max_time:.4f}s"
        )
    ...
    def test_order_execution_engine_initialization_performance(self):
        ...
        print(
            f"OrderExecutionEngine Init - Avg: {avg_time:.4f}s, Median: {median_time:.4f}s, Max: {max_time:.4f}s"
        )
    ...
    def test_trigger_condition_performance(self):
        ...
        assert avg_time_per_eval < 0.0001, (
            f"Trigger evaluation too slow: {avg_time_per_eval:.6f}s per evaluation"
        )
        assert total_time < 1.0, (
            f"Total evaluation time too slow: {total_time:.4f}s for {len(test_prices)} evaluations"
        )

        print(
            f"Trigger Evaluation - Total: {total_time:.4f}s, Per eval: {avg_time_per_eval:.6f}s"
        )
    ...
    def test_order_create_validation_performance(self):
        ...
        assert avg_time < 0.001, (
            f"Average order creation time too slow: {avg_time:.6f}s"
        )
        assert max_time < 0.01, f"Maximum order creation time too slow: {max_time:.6f}s"

        print(
            f"Order Creation - Avg: {avg_time:.6f}s, Median: {median_time:.6f}s, Max: {max_time:.6f}s"
        )
    ...
    async def test_quote_retrieval_performance(self):
        ...
        assert avg_time < 0.01, (
            f"Average quote retrieval time too slow: {avg_time:.6f}s"
        )
        assert max_time < 0.05, (
            f"Maximum quote retrieval time too slow: {max_time:.6f}s"
        )

        print(
            f"Quote Retrieval - Avg: {avg_time:.6f}s, Median: {median_time:.6f}s, Max: {max_time:.6f}s"
        )
    ...
    def test_bulk_order_creation_performance(self):
        ...
        assert total_time < 10.0, (
            f"Bulk order creation too slow: {total_time:.4f}s for {order_count} orders"
        )
        assert avg_time_per_order < 0.001, (
            f"Average order creation too slow: {avg_time_per_order:.6f}s per order"
        )

        print(
            f"Bulk Order Creation - Total: {total_time:.4f}s, Per order: {avg_time_per_order:.6f}s, Orders/sec: {order_count / total_time:.0f}"
        )

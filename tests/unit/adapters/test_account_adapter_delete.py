...
from app.models.database.trading import Account as DBAccount, Position as DBPosition, Order as DBOrder, Transaction as DBTransaction
from app.schemas.accounts import Account
from app.schemas.orders import OrderType, OrderStatus
...
class TestAccountAdapterDeleteCRUD:
    ...
    async def test_delete_existing_account_success(self, adapter: DatabaseAccountAdapter, sample_account: DBAccount, db_session: AsyncSession):
    ...
    async def test_delete_nonexistent_account(self, adapter: DatabaseAccountAdapter, db_session: AsyncSession):
    ...
    async def test_delete_account_with_positions_cascade(self, adapter: DatabaseAccountAdapter, sample_account_with_positions, db_session: AsyncSession):
    ...
    async def test_delete_account_with_all_relations_cascade(self, adapter: DatabaseAccountAdapter, sample_account_with_all_relations, db_session: AsyncSession):
        ...
        stmt = select(func.count(DBPosition.id)).filter(DBPosition.account_id == account_id)
        ...
        stmt = select(func.count(DBTransaction.id)).filter(DBTransaction.account_id == account_id)
    ...
    async def test_delete_account_transaction_integrity(self, adapter: DatabaseAccountAdapter, sample_account: DBAccount, db_session: AsyncSession):
        ...
        with patch('app.adapters.accounts.get_async_session') as mock_get_session:
            ...
            mock_db.execute.return_value.scalar_one_or_none.return_value = sample_account
    ...
    async def test_delete_account_return_values(self, adapter: DatabaseAccountAdapter, db_session: AsyncSession):
    ...
    async def test_delete_account_database_consistency(self, adapter: DatabaseAccountAdapter, db_session: AsyncSession):
    ...
    async def test_delete_account_with_empty_string_id(self, adapter: DatabaseAccountAdapter, db_session: AsyncSession):
    ...
    async def test_delete_account_with_none_id(self, adapter: DatabaseAccountAdapter, db_session: AsyncSession):
    ...
    async def test_delete_account_idempotent_behavior(self, adapter: DatabaseAccountAdapter, sample_account: DBAccount, db_session: AsyncSession):

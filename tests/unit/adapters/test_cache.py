...
class TestCacheEntry:
    ...
    def test_cache_entry_creation(self, sample_quote):
        ...
        entry = CacheEntry(value=sample_quote, timestamp=time.time(), ttl=60.0)
    ...
    def test_cache_entry_age_seconds(self, sample_quote):
        ...
        entry = CacheEntry(value=sample_quote, timestamp=start_time, ttl=60.0)
...
class TestCachedQuoteAdapter:
    ...
    @pytest.mark.asyncio
    async def test_get_quotes_cache_performance(
        self, cached_adapter, base_adapter, sample_stock, sample_quote
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_quotes_mixed_cache_state(
        self, cached_adapter, base_adapter, sample_quote
    ):
        ...
        # Pre-cache one quote
        with patch(
            "app.adapters.cache.asset_factory", side_effect=[stock1, stock1, stock2]
        ):
...
class TestCacheErrorHandling:
    ...
    def test_cached_adapter_with_adapter_error(self):
        ...
        with patch(
            "app.adapters.cache.asset_factory",
            return_value=Stock(symbol="AAPL", name="Apple"),
        ):

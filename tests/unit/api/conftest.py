...
from app.models.quotes import OptionsChainResponse, GreeksResponse, OptionQuote
...
class MockDataFactory:
    ...
    def create_option_quote(
        symbol: str = "AAPL_230616C00150000",
        price: float = 5.25,
        **kwargs
    ) -> OptionQuote:
...
class MockServiceFactory:
    ...
    def create_mock_trading_service() -> MagicMock:
        ...
        mock_service.get_portfolio_summary.return_value = MockDataFactory.create_portfolio_summary()
        ...
        mock_service.get_stock_price.return_value = MockDataFactory.create_market_data_response(data_type="price")
        mock_service.get_stock_info.return_value = MockDataFactory.create_market_data_response(data_type="info")
        mock_service.get_price_history.return_value = MockDataFactory.create_market_data_response(data_type="history")
        mock_service.get_stock_news.return_value = MockDataFactory.create_market_data_response(data_type="news")
        
        # Configure options methods
        mock_service.get_formatted_options_chain.return_value = MockDataFactory.create_options_chain_response()
        mock_service.get_expiration_dates.return_value = [date(2023, 6, 16), date(2023, 6, 23)]
        mock_service.get_option_greeks_response.return_value = MockDataFactory.create_greeks_response()
...
def sample_stock_symbols():
    ...
    return ["AAPL", "GOOGL", "MSFT", "TSLA", "AMZN", "NVDA", "META", "BRK.A", "SPY", "QQQ"]
...
def sample_order_data():
    ...
    return {
        ...
        "multi_leg_order": {
            "legs": [
                {"asset": "AAPL_230616C00150000", "order_type": "buy_to_open", "quantity": 1, "price": 5.25},
                {"asset": "AAPL_230616C00160000", "order_type": "sell_to_open", "quantity": 1, "price": 2.50}
            ],
            "condition": "limit",
            "limit_price": -2.75
        }
...
def sample_portfolio_data(mock_data_factory):
    ...
    return {
        ...
        "large_portfolio": mock_data_factory.create_portfolio(
            ...
            positions=[
                mock_data_factory.create_position(symbol, 100 + i * 10, current_price=150.0 + i * 5)
...
class APITestUtils:
    ...
    @staticmethod
    def assert_success_response(response, expected_fields: List[str] = None):
...
pytest.mark.integration = pytest.mark.mark("integration", "Integration tests requiring external services")
...
class TestHealthEndpoints:
    ...
    async def test_readiness_check_database_unhealthy(self, client):
        ...
        assert response.status_code == status.HTTP_200_OK  # Readiness still returns 200, but different status
        data = response.json()
    ...
    async def test_check_database_health_success(self):
        ...
        from app.api.v1.endpoints.health import check_database_health, HealthStatus
    ...
    async def test_check_quote_adapter_health_success(self):
        ...
        from app.api.v1.endpoints.health import check_quote_adapter_health, HealthStatus
        ...
        with patch('app.api.v1.endpoints.health.get_adapter_factory', return_value=mock_factory):
    ...
    async def test_check_trading_service_health_success(self):
        ...
        from app.api.v1.endpoints.health import check_trading_service_health, HealthStatus
        ...
        with patch('app.api.v1.endpoints.health.TradingService', return_value=mock_service):
    ...
    async def test_detailed_health_check_all_healthy(self, client):
        ...
        with patch('app.api.v1.endpoints.health.check_database_health', return_value=mock_db_health), \
             patch('app.api.v1.endpoints.health.check_quote_adapter_health', return_value=mock_quote_health), \
             patch('app.api.v1.endpoints.health.check_trading_service_health', return_value=mock_service_health):
    ...
    async def test_health_metrics_unhealthy_components(self, client):
        ...
        db_metric = next((line for line in metrics_lines if "health_database_up" in line), "")
        assert "0" in db_metric  # 0 for unhealthy
        
        quote_metric = next((line for line in metrics_lines if "health_quote_adapter_up" in line), "")
        assert "1" in quote_metric  # 1 for healthy
        assert "robinhood" in quote_metric
        
        service_metric = next((line for line in metrics_lines if "health_trading_service_up" in line), "")
        assert "0" in service_metric  # 0 for unhealthy
        
        overall_metric = next((line for line in metrics_lines if "health_overall_up" in line), "")
    ...
    async def test_dependency_health_check_quote_adapter_types(self, client):
        ...
        quote_dep = next((dep for dep in data["dependencies"] if "Quote Provider" in dep["name"]), None)
    ...
    async def test_health_check_response_times_measured(self):
        ...
        assert result["response_time_ms"] <= actual_time + 50  # Upper bound with tolerance
...
class TestTradingServiceThreadSafety:
    ...
    @pytest.mark.asyncio
    async def test_concurrent_service_initialization_same_owner(self, db_session: AsyncSession):
        ...
        with patch('app.storage.database.get_async_session') as mock_get_session:
            ...
            # Analyze results
            successful_inits = [r for r in results if isinstance(r, dict) and r["initialized"]]
            failed_inits = [r for r in results if isinstance(r, dict) and not r["initialized"]]
            ...
            assert len(account_ids) == 1, f"Expected 1 unique account ID, got {len(account_ids)}: {account_ids}"
            
            # All should report the same balance
            balances = {r["balance"] for r in successful_inits}
            assert len(balances) == 1, f"Expected 1 unique balance, got {len(balances)}: {balances}"
            ...
            assert len(accounts) == 1, f"Expected 1 account in database, found {len(accounts)}"
    ...
    async def test_concurrent_order_creation(self, db_session: AsyncSession):
        ...
        with patch('app.storage.database.get_async_session') as mock_get_session:
            ...
            assert len(order_ids) == len(successful_orders), "All orders should have unique IDs"
            ...
            assert len(db_orders) == len(successful_orders), "Database should contain all successful orders"
    ...
    async def test_concurrent_portfolio_access(self, db_session: AsyncSession):
        ...
        with patch('app.storage.database.get_async_session') as mock_get_session:
            ...
            async def access_portfolio_concurrently(access_id: int) -> Dict[str, Any]:
                ...
                try:
                    ...
                    if access_id % 3 == 0:
                        ...
                        result["portfolio_value"] = sum(pos.quantity * (pos.current_price or 0) for pos in positions)
            ...
            # Analyze results
            successful_accesses = [r for r in results if isinstance(r, dict) and r["success"]]
            failed_accesses = [r for r in results if isinstance(r, dict) and not r["success"]]
        ...
        with patch('app.storage.database.get_async_session') as mock_get_session:
            ...
            async def update_balance_concurrently(update_id: int) -> Dict[str, Any]:
                ...
                try:
                    ...
                    change_amount = (update_id % 2 * 2 - 1) * (100 + update_id * 10)  # Alternating +/- amounts
            ...
            assert abs(final_balance - expected_balance) < 0.01, f"Balance inconsistency detected"
    ...
    def test_thread_pool_trading_service_operations(self):
        ...
        assert len(successful_threads) == num_threads, f"Expected {num_threads} successful threads"
        
        # Each thread should complete its operations
        for result in successful_threads:
            assert result["operations_completed"] > 0, f"Thread {result['thread_id']} completed no operations"
...
class TestRaceConditionScenarios:
    ...
    @pytest.mark.asyncio
    async def test_account_initialization_race_condition(self, db_session: AsyncSession):
        """Test the specific race condition in _ensure_account_exists method."""
        owner_id = f"race_init_owner_{uuid.uuid4().hex[:8]}"
    ...
    async def test_order_sequence_race_condition(self, db_session: AsyncSession):
        ...
        with patch('app.storage.database.get_async_session') as mock_get_session:
            ...
            if total_cost > initial_balance:
                print("WARNING: Potential race condition - orders exceed available balance!")
            
            # At minimum, should be able to create some orders
            assert len(successful_orders) >= 1, "Should be able to create at least one order"

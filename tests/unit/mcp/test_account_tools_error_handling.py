...
class TestMCPAccountToolsErrorHandling:
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_account_info_database_error(self, mock_get_service, mock_trading_service):
        """Test account_info with database error."""
        mock_get_service.return_value = mock_trading_service
        mock_trading_service._get_account.side_effect = DatabaseError("Database connection failed", None, None)
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_account_info_account_not_found(self, mock_get_service, mock_trading_service):
        """Test account_info when account is not found."""
        mock_get_service.return_value = mock_trading_service
        mock_trading_service._get_account.side_effect = NotFoundError("Account not found")
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_account_info_missing_account_attributes(self, mock_get_service, mock_trading_service):
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_portfolio_service_timeout(self, mock_get_service, mock_trading_service):
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_portfolio_corrupted_position_data(self, mock_get_service, mock_trading_service):
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_account_details_partial_service_failure(self, mock_get_service, mock_trading_service):
        ...
        mock_trading_service.get_portfolio_summary.side_effect = Exception("Summary service failed")
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_positions_empty_result_handling(self, mock_get_service, mock_trading_service):
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_positions_invalid_position_calculation(self, mock_get_service, mock_trading_service):
    ...
    @patch('app.mcp.account_tools.get_trading_service')
    @pytest.mark.asyncio
    async def test_positions_network_interruption(self, mock_get_service, mock_trading_service):
        """Test positions with network interruption."""
        mock_get_service.return_value = mock_trading_service
        mock_trading_service.get_positions.side_effect = ConnectionError("Network interrupted")
...
class TestMCPDataIntegrityValidation:
    ...
    async def test_account_info_data_completeness(self, mock_get_service):
        ...
        required_fields = [
            "account_id", "account_type", "status", "owner", 
            "created_at", "updated_at", "cash_balance"
        ]
    ...
    async def test_positions_percentage_calculation_edge_cases(self, mock_get_service):
        ...
        edge_case_positions = [
            # Zero avg_price
            MagicMock(symbol="TEST1", quantity=100, avg_price=0, current_price=10, 
                     unrealized_pnl=1000, realized_pnl=0),
            # Zero quantity
            MagicMock(symbol="TEST2", quantity=0, avg_price=100, current_price=110, 
                     unrealized_pnl=0, realized_pnl=0),
            # Negative quantity (short position)
            MagicMock(symbol="TEST3", quantity=-50, avg_price=200, current_price=180, 
                     unrealized_pnl=1000, realized_pnl=0),
            # None values
            MagicMock(symbol="TEST4", quantity=100, avg_price=None, current_price=100, 
        ...
        for i, pos in enumerate(positions_data):
            ...
            assert not (pos["unrealized_pnl_percent"] != pos["unrealized_pnl_percent"])  # NaN check
...
class TestMCPConcurrencyErrorHandling:
    ...
    async def test_service_state_inconsistency(self, mock_get_service):
        ...
        service.get_portfolio_summary.side_effect = Exception("Service state inconsistent")

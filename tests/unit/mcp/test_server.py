...
class TestMCPToolRegistration:
    ...
    def test_registered_tools_from_tools_module(self):
        ...
        for tool in tools:
            ...
            assert inspect.iscoroutinefunction(tool), f"Tool {tool.__name__} is not async"
    ...
    def test_registered_tools_from_market_data_tools_module(self):
        ...
        for tool in market_data_tools:
            ...
            assert inspect.iscoroutinefunction(tool), f"Market data tool {tool.__name__} is not async"
...
class TestMCPServerToolCoverage:
    ...
    def test_market_data_tools_coverage(self):
        ...
        for tool_name in expected_market_data_tools:
            assert hasattr(market_data_tools, tool_name), f"Missing market data tool: {tool_name}"
...
class TestMCPServerConfiguration:
    ...
    def test_unified_architecture_pattern(self):
        ...
        assert ("unified" in source.lower() or 
                "combined" in source.lower() or
                "all MCP tools" in source)
    ...
    def test_deprecated_robinhood_integration_note(self):
        ...
        assert ("Removed direct Robinhood" in source or
                "robinhood" in source.lower())
...
class TestMCPServerToolRegistrationDetails:
    ...
    def test_tool_registration_pattern(self):
        ...
        assert tool_registrations > 15, f"Expected >15 tool registrations, found {tool_registrations}"
    ...
    def test_async_tool_registration(self):
        ...
        async_tools = [get_stock_quote, create_buy_order, get_stock_price, get_stock_info]
    ...
    def test_tool_registration_order(self):
        ...
        if basic_tools_index is not None and options_tools_index is not None:
            assert basic_tools_index < options_tools_index, "Basic tools should be registered before options tools"
...
class TestMCPServerDocumentation:
    ...
    def test_inline_comments(self):
        ...
        assert comment_found >= 2, f"Expected at least 2 explanatory comments, found {comment_found}"
...
class TestMCPServerCoverage:
    ...
    def test_all_imports_are_used(self):
        ...
        for name in imported_names:
            if name not in ['FastMCP']:  # Skip FastMCP as it's used differently
                assert name in source, f"Imported name {name} should be used in registrations"
    ...
    def test_server_module_structure(self):
        ...
        module_vars = [name for name in dir(server_module) 
                      if not name.startswith('_') and name not in expected_attrs]
        
        # Only 'mcp' should be the main export
        non_import_vars = [var for var in module_vars 
                          if not callable(getattr(server_module, var, None))]
        assert len(non_import_vars) <= 1, f"Unexpected module variables: {non_import_vars}"
    ...
    def test_registration_completeness(self):
        ...
        import_count = source.count("from app.mcp.")
        ...
        assert registration_count > 10, f"Should have many tool registrations, found {registration_count}"

...
class TestExpirationResultModel:
    ...
    def test_expiration_result_with_data(self):
        ...
        position = Position(
            symbol="AAPL", quantity=0, avg_price=145.0, current_price=150.0
        )
...
class TestFindExpiredPositions:
    ...
    def test_find_expired_positions_zero_quantity(self, expiration_engine):
        ...
        positions = [
            ...
            {
                "symbol": "GOOGL__230120P02800000",
                "quantity": 2,
            },  # Expired with quantity
        ]
...
class TestAddPosition:
    ...
    def test_add_position_existing_symbol(self, expiration_engine):
        ...
        account = {
            "positions": [{"symbol": "AAPL", "quantity": 100, "avg_price": 140.0}]
        }
...
class TestLongCallExercise:
    ...
    def test_exercise_long_call(self, expiration_engine):
        ...
        call = Call(
            underlying=Stock("AAPL"), strike=140.0, expiration_date=date(2023, 1, 20)
        )
    ...
    def test_exercise_long_call_single_contract(self, expiration_engine):
        ...
        call = Call(
            underlying=Stock("AAPL"), strike=145.0, expiration_date=date(2023, 1, 20)
        )
...
class TestShortCallAssignment:
    ...
    def test_assign_short_call_with_shares(self, expiration_engine):
        ...
        account = {
            "cash_balance": 10000.0,
            "positions": [{"symbol": "AAPL", "quantity": 300, "avg_price": 140.0}],
        }
        call = Call(
            underlying=Stock("AAPL"), strike=145.0, expiration_date=date(2023, 1, 20)
        )
...
class TestAccountExpirationsProcessing:
    ...
    @pytest.mark.asyncio
    async def test_process_account_expirations_no_expired(
        self, expiration_engine, mock_quote_adapter
    ):
    ...
    @pytest.mark.asyncio
    async def test_process_account_expirations_with_expired(
        self, expiration_engine, mock_quote_adapter
    ):
    ...
    async def test_process_account_expirations_default_date(
        ...
        await expiration_engine.process_account_expirations(
            account,
            mock_quote_adapter,  # No date provided
        )
    ...
    @pytest.mark.asyncio
    async def test_process_account_expirations_account_deepcopy(
        self, expiration_engine, mock_quote_adapter
    ):
        ...
        account_copy["positions"] = [
            pos.copy() for pos in original_account["positions"]
        ]
...
class TestErrorHandlingAndEdgeCases:
    ...
    def test_corrupted_position_data(self, expiration_engine):
        ...
        # Should handle gracefully without crashing
        try:
            expired = expiration_engine._find_expired_positions(
                positions, date(2023, 1, 21)
            )
...
class TestPerformanceAndIntegration:
    ...
    @pytest.mark.asyncio
    async def test_large_number_of_positions(
        self, expiration_engine, mock_quote_adapter
    ):
    ...
    async def test_mixed_option_types_and_underlyings(self, expiration_engine):
        ...
        multi_symbol_adapter = MockQuoteAdapter(
            {"AAPL": 150.0, "GOOGL": 2800.0, "TSLA": 200.0}
        )
        ...
        account = {
            ...
            "positions": [
                ...
                {
                    "symbol": "GOOGL__230120C02900000",
                    "quantity": 1,
                    "avg_price": 50.0,
                },  # OTM
                {
                    "symbol": "GOOGL__230120P02700000",
                    "quantity": 1,
                    "avg_price": 100.0,
                },  # ITM
                # TSLA positions
                {
                    "symbol": "TSLA__230120C00250000",
                    "quantity": -2,
                    "avg_price": 30.0,
                },  # OTM short
            ],
    ...
    async def test_complex_assignment_scenarios(self, expiration_engine):
        ...
        assert (
            "insufficient" in result.warnings[0].lower()
            or "forced" in result.assignments[0].get("warning", "").lower()
        )

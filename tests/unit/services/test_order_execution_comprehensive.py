...
class TestOrderExecutionResult:
    ...
    def test_execution_result_with_data(self):
        ...
        position = Position(
            symbol="AAPL", quantity=100, avg_price=150.0, current_price=152.0
        )
    ...
    def test_execution_result_failure(self):
        ...
        result = OrderExecutionResult(
            success=False, message="Insufficient funds", order_id="order123"
        )
...
class TestOrderValidation:
    ...
    def test_validate_order_no_legs(self, execution_engine):
        ...
        order = MultiLegOrder(id="order123", legs=[], condition=OrderCondition.MARKET)
...
class TestLegPriceCalculation:
    ...
    @pytest.mark.asyncio
    async def test_calculate_leg_prices_with_quote_service(
        self, execution_engine, sample_stock
    ):
        ...
        leg = OrderLeg(
            asset=sample_stock, quantity=100, order_type=OrderType.BTO, price=150.0
        )
    ...
    @pytest.mark.asyncio
    async def test_calculate_leg_prices_without_quote_service(
        self, execution_engine, sample_stock
    ):
        """Test calculating leg prices without quote service (fallback mode)."""
        leg = OrderLeg(
            asset=sample_stock, quantity=100, order_type=OrderType.BTO, price=145.0
        )
    ...
    @pytest.mark.asyncio
    async def test_calculate_leg_prices_negative_quantity(
        self, execution_engine, sample_stock
    ):
...
class TestOrderFillConditions:
    ...
    def test_should_fill_order_limit_fill(self, execution_engine):
        ...
        order = MultiLegOrder(
            id="order123", legs=[], condition=OrderCondition.LIMIT, limit_price=155.0
        )
    ...
    def test_should_fill_order_limit_no_fill(self, execution_engine):
        ...
        order = MultiLegOrder(
            id="order123", legs=[], condition=OrderCondition.LIMIT, limit_price=145.0
        )
    ...
    def test_should_fill_order_limit_no_price(self, execution_engine):
        ...
        order = MultiLegOrder(
            id="order123", legs=[], condition=OrderCondition.LIMIT, limit_price=None
        )
...
class TestClosingPositionValidation:
    ...
    def test_validate_closing_positions_success(
        self, execution_engine, sample_stock, sample_positions
    ):
    ...
    def test_validate_closing_positions_no_position(
        self, execution_engine, sample_positions
    ):
        ...
        with pytest.raises(
            OrderExecutionError, match="No available positions to close"
        ):
            execution_engine._validate_closing_positions([leg], sample_positions)

    def test_validate_closing_positions_insufficient_quantity(
        self, execution_engine, sample_stock, sample_positions
    ):
    ...
    def test_validate_closing_positions_wrong_direction(
        self, execution_engine, sample_stock, sample_positions
    ):
...
class TestCashRequirementCalculation:
    ...
    def test_calculate_cash_requirement_buy_stock(self, execution_engine, sample_stock):
        ...
        cash_requirement = execution_engine._calculate_cash_requirement(
            [leg], leg_prices
        )
    ...
    def test_calculate_cash_requirement_sell_stock(
        self, execution_engine, sample_stock
    ):
        """Test cash requirement for selling stock."""
        leg = OrderLeg(
            asset=sample_stock, quantity=-100, order_type=OrderType.STO, price=150.0
        )
    ...
    def test_calculate_cash_requirement_buy_option(
        self, execution_engine, sample_call_option
    ):
        """Test cash requirement for buying option."""
        leg = OrderLeg(
            asset=sample_call_option, quantity=2, order_type=OrderType.BTO, price=10.0
        )
    ...
    def test_calculate_cash_requirement_sell_option(
        self, execution_engine, sample_put_option
    ):
        """Test cash requirement for selling option."""
        leg = OrderLeg(
            asset=sample_put_option, quantity=-1, order_type=OrderType.STO, price=8.0
        )
    ...
    def test_calculate_cash_requirement_invalid_buy_signs(
        self, execution_engine, sample_stock
    ):
        ...
        with pytest.raises(
            OrderExecutionError, match="Buy orders must have positive quantity"
        ):
            execution_engine._calculate_cash_requirement([leg], leg_prices)

    def test_calculate_cash_requirement_invalid_sell_signs(
        self, execution_engine, sample_stock
    ):
        ...
        with pytest.raises(
            OrderExecutionError, match="Sell orders must have negative quantity"
        ):
            execution_engine._calculate_cash_requirement([leg], leg_prices)
...
class TestOpenPosition:
    ...
    async def test_open_position_option(self, execution_engine, sample_call_option):
        ...
        leg = OrderLeg(
            asset=sample_call_option, quantity=2, order_type=OrderType.BTO, price=12.0
        )
    ...
    @pytest.mark.asyncio
    async def test_open_position_with_quote_service(
        self, execution_engine, sample_stock
    ):
...
class TestClosePosition:
    ...
    def test_close_position_partial(
        ...
        modified_positions = execution_engine._close_position(
            leg, sample_positions, -155.0
        )
    ...
    def test_close_position_complete(
        self, execution_engine, sample_stock, sample_positions
    ):
...
class TestMultiLegOrderExecution:
    ...
    @pytest.mark.asyncio
    async def test_execute_order_closing_position(
        self, execution_engine, sample_stock, sample_positions
    ):
...
class TestErrorHandling:
    ...
    def test_cash_requirement_zero_quantity(self, execution_engine, sample_stock):
        ...
        leg = OrderLeg(
            asset=sample_stock, quantity=0, order_type=OrderType.BTO, price=150.0
        )
    ...
    def test_close_position_no_matching_positions(
        self, execution_engine, sample_positions
    ):
        """Test closing position when no matching positions exist."""
        tsla_stock = Stock("TSLA")
        leg = OrderLeg(
            asset=tsla_stock, quantity=-100, order_type=OrderType.STC, price=800.0
        )

        # Should return empty list when no matching positions
        modified_positions = execution_engine._close_position(
            leg, sample_positions, -800.0
        )
...
class TestIntegrationScenarios:
    ...
    async def test_covered_call_strategy(self, execution_engine):
        ...
        stock = Stock("AAPL")
        call = Call(
            underlying=stock, strike=155.0, expiration_date=datetime(2023, 6, 16).date()
        )
    ...
    async def test_protective_put_strategy(self, execution_engine):
        ...
        stock = Stock("AAPL")
        put = Put(
            underlying=stock, strike=140.0, expiration_date=datetime(2023, 6, 16).date()
        )
    ...
    async def test_spread_strategy(self, execution_engine):
        ...
        stock = Stock("AAPL")
        call_lower = Call(
            underlying=stock, strike=145.0, expiration_date=datetime(2023, 6, 16).date()
        )
        call_higher = Call(
            underlying=stock, strike=155.0, expiration_date=datetime(2023, 6, 16).date()
        )
...
class TestPerformanceScenarios:
    ...
    async def test_large_multi_leg_order(self, execution_engine):
        ...
        for i in range(10):
            ...
            leg = OrderLeg(
                asset=stock, quantity=100, order_type=OrderType.BTO, price=100.0 + i
            )
            legs.append(leg)

        order = MultiLegOrder(id="order123", legs=legs, condition=OrderCondition.MARKET)
    ...
    @pytest.mark.asyncio
    async def test_execution_with_many_existing_positions(
        self, execution_engine, sample_stock
    ):

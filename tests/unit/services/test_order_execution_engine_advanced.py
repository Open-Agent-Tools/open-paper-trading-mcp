...
class TestOrderExecutionEngine:
    ...
    async def test_check_trigger_conditions_with_trigger(self):
        ...
        with patch('app.services.order_execution_engine._get_quote_adapter') as mock_get_adapter:
            mock_adapter = AsyncMock()
            mock_get_adapter.return_value = mock_adapter
            
            with patch('app.services.order_execution_engine.asset_factory') as mock_factory:
                ...
                with patch.object(self.engine, '_process_triggered_order') as mock_process:
                    await self.engine._check_trigger_conditions()
    ...
    async def test_process_triggered_order_stop_loss(self):
        ...
        with patch.object(self.engine, '_load_order_by_id') as mock_load:
            ...
            with patch('app.services.order_execution_engine.order_converter') as mock_converter:
                ...
                with patch.object(self.engine, '_update_order_triggered_status') as mock_update:
                    with patch.object(self.engine, '_execute_converted_order') as mock_execute:
                        await self.engine._process_triggered_order(condition, 144.0)
                        
                        mock_converter.convert_stop_loss_to_market.assert_called_once_with(mock_order, 144.0)
            ...
            with patch('app.services.order_execution_engine.order_converter') as mock_converter:
                ...
                with patch.object(self.engine, '_update_order_triggered_status') as mock_update:
                    with patch.object(self.engine, '_execute_converted_order') as mock_execute:
                        ...
                        mock_converter.convert_stop_limit_to_limit.assert_called_once_with(mock_order, 144.0)
                        mock_update.assert_called_once()
                        mock_execute.assert_called_once_with(mock_converted)
    ...
    async def test_process_triggered_order_trailing_stop(self):
        ...
        with patch.object(self.engine, '_load_order_by_id') as mock_load:
            ...
            with patch('app.services.order_execution_engine.order_converter') as mock_converter:
                mock_converted = Mock()
                mock_converter.convert_trailing_stop_to_market.return_value = mock_converted
                
                with patch.object(self.engine, '_update_order_triggered_status') as mock_update:
                    with patch.object(self.engine, '_execute_converted_order') as mock_execute:
                        await self.engine._process_triggered_order(condition, 144.0)
                        
                        mock_converter.convert_trailing_stop_to_market.assert_called_once_with(mock_order, 144.0)
                        mock_update.assert_called_once()
                        mock_execute.assert_called_once_with(mock_converted)
    ...
    async def test_process_triggered_order_conversion_error(self):
        ...
        with patch.object(self.engine, '_load_order_by_id') as mock_load:
            ...
            with patch('app.services.order_execution_engine.order_converter') as mock_converter:
                mock_converter.convert_stop_loss_to_market.side_effect = Exception("Conversion error")
    ...
    async def test_execute_converted_order_execution_error(self):
        ...
        self.mock_trading_service.execute_order = AsyncMock()
        self.mock_trading_service.execute_order.side_effect = Exception("Execution error")
    ...
    async def test_monitoring_loop_integration(self):
        ...
        with patch.object(self.engine, '_check_trigger_conditions') as mock_check:
            mock_check.side_effect = [None, asyncio.CancelledError()]  # Run once then cancel
    ...
    async def test_concurrent_order_processing(self):
        ...
        with patch('app.services.order_execution_engine.order_converter') as mock_converter:
            ...
            total_conditions = sum(
                len(conditions) for conditions in self.engine.trigger_conditions.values()
            )
...
class TestOrderExecutionEngineErrorScenarios:
    ...
    async def test_corrupted_order_data(self):
        ...
        with patch('app.services.order_execution_engine.order_converter') as mock_converter:
            mock_converter.can_convert_order.return_value = True
            mock_converter.validate_order_for_conversion.side_effect = ValueError("Missing ID")

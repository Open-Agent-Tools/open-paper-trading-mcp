...
class TestTriggerConditionChecking:
    ...
    @pytest.mark.asyncio
    async def test_check_trigger_conditions_with_symbol_and_price(
        ...
        with patch.object(
            ...
            assert (
                len(execution_engine.trigger_conditions["AAPL"]) == 0
...
class TestOrderProcessing:
    ...
    async def test_process_triggered_order_exception(self, execution_engine):
        ...
        with patch.object(
            execution_engine, "_load_order_by_id", side_effect=Exception("DB error")
        ):
...
class TestEngineStatus:
    ...
    def test_get_status(self, execution_engine):
        ...
        condition1 = TriggerCondition(
            "order1", "AAPL", "stop_loss", 145.0, OrderType.SELL
        )
        condition2 = TriggerCondition(
            "order2", "GOOGL", "stop_limit", 2800.0, OrderType.SELL
        )
    ...
    def test_get_monitored_orders(self, execution_engine):
        ...
        condition2 = TriggerCondition(
            "order2", "AAPL", "trailing_stop", 150.0, OrderType.SELL
        )
...
class TestIntegrationScenarios:
    ...
    async def test_high_frequency_price_updates(self, execution_engine):
        ...
        with patch.object(
            ...
            side_effect=mock_process_triggered_order,
        ):
            for price in prices:
...
class TestPerformanceScenarios:
    ...
    async def test_many_monitored_symbols(self, execution_engine):
        ...
        for i in range(100):
            symbol = f"STOCK{i}"
            condition = TriggerCondition(
                f"order{i}", symbol, "stop_loss", 100.0 + i, OrderType.SELL
            )
    ...
    async def test_many_conditions_per_symbol(self, execution_engine):
        ...
        for i in range(50):
            condition = TriggerCondition(
                f"order{i}", "AAPL", "stop_loss", 100.0 + i, OrderType.SELL
            )
            execution_engine.trigger_conditions["AAPL"].append(condition)

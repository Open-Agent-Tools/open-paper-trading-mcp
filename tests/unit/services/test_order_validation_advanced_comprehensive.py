...
class TestComplexOrderValidator:
    ...
    def vertical_spread_order(self):
        ...
        legs = [
            OrderLeg(asset=buy_call, quantity=1, order_type=OrderType.BTO, price=5.0),
            OrderLeg(asset=sell_call, quantity=-1, order_type=OrderType.STO, price=2.0),
        ]
    ...
    def straddle_order(self):
        ...
        legs = [
            ...
            OrderLeg(asset=put, quantity=1, order_type=OrderType.BTO, price=3.0),
        ]
    ...
    def test_basic_requirements_validation(self):
        ...
        empty_order_issues = [
            issue for issue in result.issues if issue.code == "EMPTY_ORDER"
        ]
    ...
    def test_zero_quantity_validation(self, sample_portfolio):
        ...
        zero_qty_issues = [
            issue for issue in result.issues if issue.code == "ZERO_QUANTITY"
        ]
    ...
    def test_duplicate_symbols_validation(self, sample_portfolio):
        ...
        legs = [
            OrderLeg(asset=call, quantity=1, order_type=OrderType.BTO, price=5.0),
            OrderLeg(
                asset=call, quantity=-1, order_type=OrderType.STC, price=5.0
            ),  # Same asset
        ]
        ...
        duplicate_issues = [
            issue for issue in result.issues if issue.code == "DUPLICATE_SYMBOLS"
        ]

        assert len(duplicate_issues) > 0

    def test_strategy_specific_validation(
        self, vertical_spread_order, sample_portfolio
    ):
        ...
        strategy_issues = [
            ...
            if issue.code
            in ["INVALID_LEG_COUNT", "MISSING_ASSET_TYPE", "DIFFERENT_UNDERLYINGS"]
        ]

        # Should have no strategy validation issues for valid spread
        assert len(strategy_issues) == 0

    def test_options_level_validation_insufficient(
        self, vertical_spread_order, sample_portfolio
    ):
    ...
    def test_options_level_validation_sufficient(
        self, vertical_spread_order, sample_portfolio
    ):
    ...
    def test_naked_options_detection(self, sample_portfolio):
        ...
        result = validator.validate_order(
            naked_order, sample_portfolio, options_level=2
        )
    ...
    def test_near_expiration_warning(self, sample_portfolio):
        ...
        legs = [OrderLeg(asset=call, quantity=1, order_type=OrderType.BTO, price=0.5)]
        ...
        expiry_warnings = [
            warning for warning in result.warnings if warning.code == "NEAR_EXPIRATION"
        ]
    ...
    def test_expiration_day_restriction(self, mock_date, sample_portfolio):
        ...
        expiry_issues = [
            issue for issue in result.issues if issue.code == "EXPIRATION_DAY_SHORT"
        ]

        assert len(expiry_issues) > 0
        assert expiry_issues[0].severity == "error"

    def test_margin_requirement_calculation_stock(
        self, covered_call_order, sample_portfolio
    ):
        ...
        assert (
            result.margin_requirement > expected_stock_margin - expected_option_premium
        )

    def test_margin_requirement_calculation_options(
        self, vertical_spread_order, sample_portfolio
    ):
    ...
    def test_max_profit_loss_vertical_spread(
        self, vertical_spread_order, sample_portfolio
    ):
    ...
    def test_breakeven_calculation_vertical_spread(
        self, vertical_spread_order, sample_portfolio
    ):
    ...
    def test_iron_condor_pnl_calculation(self, sample_portfolio):
        ...
        legs = [
            OrderLeg(
                asset=put1, quantity=1, order_type=OrderType.BTO, price=1.0
            ),  # Buy put (lower)
            OrderLeg(
                asset=put2, quantity=-1, order_type=OrderType.STO, price=2.0
            ),  # Sell put (higher)
            OrderLeg(
                asset=call1, quantity=-1, order_type=OrderType.STO, price=3.0
            ),  # Sell call (lower)
            OrderLeg(
                asset=call2, quantity=1, order_type=OrderType.BTO, price=1.0
            ),  # Buy call (higher)
        ]
    ...
    def test_complex_three_leg_strategy(self, sample_portfolio):
        ...
        legs = [
            OrderLeg(
                asset=call1, quantity=1, order_type=OrderType.BTO, price=7.0
            ),  # Buy low strike
            OrderLeg(
                asset=call2, quantity=-2, order_type=OrderType.STO, price=4.0
            ),  # Sell 2x middle
            OrderLeg(
                asset=call3, quantity=1, order_type=OrderType.BTO, price=2.0
            ),  # Buy high strike
        ]
    ...
    def test_error_handling_invalid_asset(self, sample_portfolio):
        ...
        invalid_asset_issues = [
            issue for issue in result.issues if issue.code == "INVALID_ASSET"
        ]
...
class TestRealWorldScenarios:
    ...
    def test_insufficient_capital_scenario(self):
        ...
        # Should generate concentration warnings
        concentration_warnings = [
            w for w in result.warnings if w.code == "HIGH_CONCENTRATION"
        ]
    ...
    def test_beginner_options_level_restriction(self):
        ...
        legs = [OrderLeg(asset=call, quantity=1, order_type=OrderType.BTO, price=5.0)]

...
class TestPerformanceMonitor:
    ...
    def test_monitor_with_custom_thresholds(self):
        ...
        custom_thresholds = PerformanceThresholds(
            max_avg_latency_ms=50.0, max_p95_latency_ms=100.0, min_success_rate=0.99
        )
    ...
    def test_measurement_with_metadata(self):
        ...
        metric = monitor.end_measurement(
            measurement_id, success=True, metadata={"result": "filled"}
        )
    ...
    def test_measurement_with_error(self):
        ...
        metric = monitor.end_measurement(
            measurement_id, success=False, error="Connection timeout"
        )
    ...
    async def test_benchmark_with_all_success(self):
        ...
        result = await monitor.run_benchmark(
            "success_benchmark", success_test_function, num_operations=20, concurrency=2
        )
    ...
    def test_alert_creation(self):
        ...
        monitor._create_alert(
            "test_alert", "warning", "Test alert message", "test_metric", 100.0, 50.0
        )
    ...
    def test_metrics_export(self):
        ...
        # Add an alert
        monitor._create_alert(
            "export_alert", "warning", "Export test alert", "export_metric", 100.0, 50.0
        )
    ...
    def test_alert_list_size_management(self):
        ...
        for i in range(1200):  # More than the 1000 limit
            monitor._create_alert(
                f"alert_{i}", "warning", f"Alert {i}", f"metric_{i}", float(i), 50.0
            )
...
class TestDataClasses:
    ...
    def test_performance_thresholds(self):
        ...
        # Test custom values
        custom_thresholds = PerformanceThresholds(
            max_avg_latency_ms=50.0, min_success_rate=0.99
        )
...
class TestMeasurementContexts:
    ...
    def test_sync_context_initialization(self):
        ...
        context = SyncMeasurementContext(monitor, "test_operation", {"key": "value"})
    ...
    def test_async_context_initialization(self):
        ...
        context = AsyncMeasurementContext(monitor, "async_test", {"async": True})
...
class TestRealWorldScenarios:
    ...
    def test_performance_degradation_detection(self):
        ...
        thresholds = PerformanceThresholds(
            max_avg_latency_ms=10.0, min_success_rate=0.95
        )
    ...
    async def test_benchmark_comparison(self):
        ...
        old_result = await monitor.run_benchmark(
            "old_implementation", old_implementation, num_operations=50, concurrency=5
        )

        new_result = await monitor.run_benchmark(
            "new_implementation", new_implementation, num_operations=50, concurrency=5
        )
    ...
    def test_resource_usage_tracking(self):
        ...
        cache_hit_ratio = monitor.gauges.get("cache_hits", 0) / (
            monitor.counters.get("cache_hits", 0)
            + monitor.counters.get("cache_misses", 1)
        )
    ...
    async def test_real_time_alerting(self):
        ...
        thresholds = PerformanceThresholds(
            max_avg_latency_ms=5.0, min_success_rate=0.98, min_throughput_ops_sec=50.0
        )

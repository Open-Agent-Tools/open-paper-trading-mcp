...
class TestRiskAnalyzer:
    ...
    def test_analyzer_with_custom_limits(self):
        ...
        custom_limits = RiskLimits(
            max_position_concentration=0.15, max_sector_exposure=0.30, max_leverage=1.5
        )
    ...
    def sample_order(self):
        ...
        return Order(symbol="AAPL", quantity=50, order_type=OrderType.BUY, price=155.0)
    ...
    def test_analyze_order_basic(self, sample_order, sample_portfolio, sample_quote):
        ...
        result = analyzer.analyze_order(sample_order, sample_portfolio, sample_quote)
    ...
    def test_portfolio_impact_calculation(
        self, sample_order, sample_portfolio, sample_quote
    ):
    ...
    def test_position_impacts_calculation(
        self, sample_order, sample_portfolio, sample_quote
    ):
    ...
    def test_single_position_impact_calculation(self, sample_portfolio, sample_quote):
        ...
        order = Order(symbol="AAPL", quantity=25, order_type=OrderType.BUY, price=155.0)
    ...
    def test_position_concentration_check(self, sample_portfolio):
        ...
        if concentration_violations:
            assert concentration_violations[0].severity in [
                RiskLevel.HIGH,
                RiskLevel.MODERATE,
            ]
    ...
    def test_leverage_check_margin_account(self, sample_portfolio):
        ...
        leverage_violations = [
            v for v in violations if v.check_type == RiskCheckType.PORTFOLIO_LEVERAGE
        ]
    ...
    def test_buying_power_check(self, sample_portfolio):
        ...
        buying_power_violations = [
            v for v in violations if v.check_type == RiskCheckType.BUYING_POWER
        ]
    ...
    def test_options_level_check(self):
        ...
        level_violations = [
            v for v in violations if v.check_type == RiskCheckType.OPTIONS_LEVEL
        ]
    ...
    def test_margin_requirement_calculation_margin(self, sample_quote):
        ...
        stock_order = Order(
            symbol="AAPL", quantity=100, order_type=OrderType.BUY, price=155.0
        )
...
class TestComplexScenarios:
    ...
    def test_complex_portfolio_analysis(self, complex_portfolio):
        ...
        large_order = Order(
            symbol="AAPL", quantity=200, order_type=OrderType.BUY, price=155.0
        )
        ...
        if concentration_violations:
            assert (
                concentration_violations[0].current_value
                > concentration_violations[0].limit_value
            )
    ...
    def test_short_selling_risk_analysis(self, complex_portfolio):
        ...
        # Short selling order
        short_order = Order(
            symbol="AMZN", quantity=-50, order_type=OrderType.SELL, price=3000.0
        )
        ...
        assert (
            result.portfolio_impact.cash_after > result.portfolio_impact.cash_before
        )  # Short selling adds cash
    ...
    def test_expiration_day_options_restriction(self, mock_datetime, complex_portfolio):
        ...
        with patch("app.models.assets.asset_factory") as mock_factory:
            ...
            result = analyzer.analyze_order(
                expiring_option_order, complex_portfolio, quote
            )

            # Should have regulatory violation for expiration day short
            regulatory_violations = [
                v for v in result.violations if "expiration day" in v.message.lower()
            ]

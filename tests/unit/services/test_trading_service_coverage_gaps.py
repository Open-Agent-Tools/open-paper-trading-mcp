...
class TestTradingServiceCoverageGaps:
    ...
    @pytest.mark.asyncio
    async def test_database_session_runtime_error_fallback(self, trading_service_test_data):
    ...
    @pytest.mark.asyncio
    async def test_get_price_history_invalid_symbol_error(self, trading_service_test_data):
    ...
    @pytest.mark.asyncio
    async def test_get_price_history_no_quote_fallback_error(self, trading_service_test_data):
        ...
        try:
            ...
            with patch.object(trading_service_test_data, 'get_enhanced_quote') as mock_quote:
            ...
            if original_method:
                trading_service_test_data.quote_adapter.get_price_history = original_method

    @pytest.mark.asyncio
    async def test_get_price_history_extended_adapter_with_none_result(self, trading_service_test_data):
        ...
        with patch.object(trading_service_test_data.quote_adapter, 'get_price_history') as mock_method:
    ...
    @pytest.mark.asyncio
    async def test_get_price_history_exception_handling(self, trading_service_test_data):
    ...
    @pytest.mark.asyncio 
    async def test_get_stock_info_adapter_extended_functionality(self, trading_service_test_data):
        ...
        with patch.object(trading_service_test_data.quote_adapter, 'get_stock_info') as mock_method:
    ...
    @pytest.mark.asyncio
    async def test_search_stocks_adapter_extended_functionality(self, trading_service_test_data):
        ...
        with patch.object(trading_service_test_data.quote_adapter, 'search_stocks') as mock_method:
    ...
    @pytest.mark.asyncio
    async def test_fallback_data_point_with_missing_volume(self, trading_service_test_data):
    ...
    async def test_get_account_not_found_error(self, trading_service_test_data):
        ...
        async def _operation(db):
            ...
            if not account:
                ...
                raise NotFoundError(f"Account for owner nonexistent_owner not found")
            return account
        
        # This should trigger the NotFoundError on line 178
        with pytest.raises(Exception):  # Could be NotFoundError or other exception
            await trading_service_test_data._execute_with_session(_operation)

    @pytest.mark.asyncio
    async def test_get_enhanced_quote_invalid_symbol_error(self, trading_service_test_data):
    ...
    @pytest.mark.asyncio
    async def test_boundary_value_testing_extreme_quantities(self, trading_service_test_data):
        ...
        try:
            ...
            assert any(word in error_str for word in ["validation", "limit", "quantity", "error"])

    @pytest.mark.asyncio
    async def test_boundary_value_testing_extreme_prices(self, trading_service_test_data):
        ...
        try:
            ...
            # Should handle gracefully
            error_str = str(e).lower()
            assert any(word in error_str for word in ["validation", "price", "precision", "error"])
        ...
        with patch.object(trading_service_test_data.quote_adapter, 'get_quote') as mock_get_quote:
            ...
            try:
                result = await asyncio.wait_for(
                    trading_service_test_data.get_enhanced_quote("AAPL"), 
                    timeout=0.1
                )
            except asyncio.TimeoutError:
                # Expected timeout - this tests timeout handling paths
                pass

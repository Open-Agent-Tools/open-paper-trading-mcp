...
from app.schemas.orders import OrderCreate, OrderStatus, OrderType, OrderCondition
...
class TestGetOrders:
    ...
    async def test_get_orders_different_statuses(self, db_session: AsyncSession):
        ...
        expected_statuses = {OrderStatus.PENDING, OrderStatus.FILLED, OrderStatus.CANCELLED}
    ...
    async def test_cancel_all_stock_orders_success(self, db_session: AsyncSession):
        ...
        for i in range(2):
            order = DBOrder(
                id=str(uuid.uuid4()),
                account_id=account.id,
                symbol=f"AAPL240115C00150000",  # Option-style symbols with C
                order_type=OrderType.BUY,
                quantity=1,
                price=5.0,
                status=OrderStatus.PENDING,
    ...
    async def test_cancel_all_option_orders_success(self, db_session: AsyncSession):
        ...
        for i in range(2):
            order = DBOrder(
                id=str(uuid.uuid4()),
                account_id=account.id,
                symbol=f"TSLA240115C00200000",  # Call options
                order_type=OrderType.BUY,
                quantity=1,
                price=5.0,
                status=OrderStatus.PENDING,
...
class TestOrderManagementErrorHandling:
    ...
    async def test_create_order_database_error(self, db_session: AsyncSession):
        ...
        with patch.object(db_session, 'commit', side_effect=DatabaseError("DB Error", None, None)):
            order_data = OrderCreate(
                symbol="AAPL",
                order_type=OrderType.BUY,
                quantity=100,
    ...
    async def test_concurrent_order_operations(self, db_session: AsyncSession):
        ...
        success_count = sum(1 for result in [get_result, cancel_result] 

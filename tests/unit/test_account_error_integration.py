...
class TestAccountErrorIntegration:
    ...
    @pytest.mark.asyncio
    async def test_api_to_database_error_propagation(
        ...
        mock_service.get_portfolio.side_effect = OperationalError(
    ...
    async def test_mcp_to_service_error_propagation(self, mock_get_service):
        ...
        mock_service.get_portfolio.side_effect = DatabaseError(
            "Database query failed", None, None
        )
        ...
        def session_side_effect(*args, **kwargs):
            ...
            async def mock_session_generator():
                ...
                if call_count == 1:
                    ...
                    account = DBAccount(
                        id="test-123", owner="test_user", cash_balance=10000.0
                    )
    ...
    async def test_adapter_database_rollback_integration(self, mock_get_session):
        ...
        mock_db = MagicMock()
        # Mock the query result for checking existing account
        mock_result = MagicMock()
        ...
        mock_db.add = MagicMock()  # Mock add method
        ...
        # Verify that commit was attempted (which then failed)
        mock_db.commit.assert_called_once()
        # Note: rollback might not be explicitly called due to context manager behavior

    @patch("app.mcp.account_tools.get_trading_service")
    @patch("app.storage.database.get_async_session")
    @pytest.mark.asyncio
    async def test_end_to_end_error_handling_chain(
        self, mock_get_session, mock_get_service
    ):
        """Test complete error handling chain from MCP to database."""
        # Setup database layer failure
        mock_session = MagicMock(spec=AsyncSession)
        mock_session.execute = AsyncMock(
            side_effect=DatabaseError("Critical database error", None, None)
        )
    ...
    async def test_concurrent_access_error_handling(self, mock_get_session):
        ...
        def session_side_effect(*args, **kwargs):
            ...
            async def mock_session_generator():
                ...
                if call_count == 1:
                    ...
                    mock_db.execute = AsyncMock(return_value=mock_result)
                    mock_db.delete = AsyncMock()  # Mock delete method
                    mock_db.commit = AsyncMock()  # Mock commit method
                    yield mock_db
                else:
                    # Second access fails due to concurrent modification
                    mock_db.execute = AsyncMock(
                        side_effect=OperationalError("Lock timeout", None, None)
                    )
                    yield mock_db
        ...
        await adapter.delete_account("test-id")
    ...
    async def test_mcp_error_response_format_integration(self, mock_get_service):
        ...
        for error, expected_type in error_scenarios:
            ...
            assert result["result"]["status"] == "error"
            assert (
                expected_type in result["result"]["error"]
                or "error" in result["result"]["error"].lower()
            )
    ...
    async def test_service_transaction_isolation(self, mock_get_session):
        ...
        mock_session = MagicMock(spec=AsyncSession)
        mock_session.execute = AsyncMock(
            side_effect=DatabaseError("Transaction failed", None, None)
        )
    ...
    async def test_database_connection_recovery_integration(self, mock_get_session):
        ...
        mock_session = MagicMock(spec=AsyncSession)
        mock_result = MagicMock()
        account = DBAccount(id="test-123", owner="test_user", cash_balance=10000.0)
        mock_result.scalar_one_or_none.return_value = account

        positions_result = MagicMock()
        positions_result.scalars.return_value.all.return_value = []

        # Create a repeatable mock that can handle multiple calls
        mock_session.execute = AsyncMock()

...
class TestQuoteCache:
    ...
    def test_cache_cleanup_expired(self, cache):
        ...
        expired_entry = CacheEntry(
            value="expired_value", timestamp=current_time - 120.0, ttl=60.0
        )
...
class MockQuoteAdapter(QuoteAdapter):
    ...
    async def get_options_chain(self, underlying, expiration_date=None):
        ...
        key = (
            f"{underlying}_{expiration_date.isoformat() if expiration_date else 'all'}"
        )
...
class TestCachedQuoteAdapter:
    ...
    def test_get_quote_cache_hit(self, cached_adapter, mock_adapter):
        ...
        assert result is quote
        assert (
            mock_adapter.call_counts["get_quote"] == 0
        )  # Should not call underlying adapter
    ...
    @pytest.mark.asyncio
    async def test_get_quotes_mixed_cache_hits_misses(
        self, cached_adapter, mock_adapter
    ):
        ...
        with patch(
            "app.models.assets.asset_factory", side_effect=asset_factory_side_effect
        ):
    ...
    async def test_get_options_chain_cache_hit(self, cached_adapter, mock_adapter):
        ...
        assert result is chain
        assert (
            mock_adapter.call_counts["get_options_chain"] == 0
...
class TestCacheUtilities:
    ...
    def test_cache_memory_usage_control(self):
        ...
        assert (
            len(cache._cache) <= cache.max_size * 1.1
        )  # Allow 10% overage for cleanup timing

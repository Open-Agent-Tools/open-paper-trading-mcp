...
def test_validate_order_success(advanced_validator, sample_account_data, sample_quotes):
    ...
    result = advanced_validator.validate_order(sample_account_data, order, sample_quotes)
    assert result.is_valid
    assert result.can_execute

def test_validate_expiration_date_too_close(advanced_validator, sample_account_data, sample_quotes):
...
def test_position_size_exceeded(advanced_validator, sample_account_data, sample_quotes):
    ...
    result = advanced_validator.validate_order(sample_account_data, order, sample_quotes, limits)
    assert not result.is_valid
    error_codes = [msg.code for msg in result.errors]

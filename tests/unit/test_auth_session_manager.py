...
class TestSessionManager:
    ...
    def test_check_circuit_breaker_open(self, session_manager):
        ...
        session_manager._circuit_breaker_reset_time = datetime.now() + timedelta(
            minutes=5
        )
    ...
    def test_check_circuit_breaker_reset(self, session_manager, mock_logger):
        ...
        mock_logger.info.assert_called_once_with(
            "Circuit breaker reset - authentication attempts resumed"
        )
    ...
    async def test_authenticate_success(
        self, session_manager, mock_robinhood, mock_logger
    ):
        ...
        mock_logger.info.assert_any_call(
            "Attempting to authenticate user: test_user (attempt 1)"
        )
    ...
    @pytest.mark.asyncio
    async def test_authenticate_profile_load_failure(
        self, session_manager, mock_robinhood
    ):
        ...
        with pytest.raises(
            AuthenticationError, match="Could not retrieve user profile"
        ):
            await session_manager._authenticate()
    ...
    @pytest.mark.asyncio
    async def test_ensure_authenticated_invalid_session(
        self, session_manager, mock_robinhood
    ):
    ...
    @pytest.mark.asyncio
    async def test_logout_with_error(
        self, session_manager, mock_robinhood, mock_logger
    ):
...
class TestSessionManagerTokenHandling:
    ...
    @pytest.mark.asyncio
    async def test_token_storage_during_authentication(
        self, session_manager, mock_robinhood
    ):
...
class TestSessionManagerAsyncBehavior:
    ...
    @pytest.mark.asyncio
    async def test_executor_usage_for_blocking_calls(
        self, session_manager, mock_robinhood
    ):

...
class TestGetTradingServiceDependency:
    ...
    def test_trading_service_not_found_raises_runtime_error(self):
        ...
        assert "Ensure the service is initialized in the lifespan context manager" in str(exc_info.value)
    ...
    def test_request_app_state_access_pattern(self):
        ...
        with patch('app.core.dependencies.getattr', return_value=mock_trading_service) as mock_getattr:
            result = get_trading_service(mock_request)
            
            # Verify getattr was called with correct parameters
            mock_getattr.assert_called_once_with(mock_request.app.state, "trading_service", None)
            assert result is mock_trading_service
...
class TestDependencyInjectionPatterns:
    ...
    def test_dependency_can_be_used_in_fastapi_endpoint(self):
        ...
        def mock_endpoint(trading_service: TradingService = Depends(get_trading_service)):
        ...
        assert hasattr(result, '__call__')  # Should return a method
    ...
    def test_dependency_with_async_endpoint(self):
        ...
        async def mock_async_endpoint(
            ...
            return await trading_service.get_portfolio() if hasattr(trading_service.get_portfolio, '__call__') else None
...
class TestApplicationStateManagement:
    ...
    def test_app_state_without_state_attribute(self):
        ...
        with patch('app.core.dependencies.getattr', side_effect=AttributeError("No state")):
...
class TestFastAPIIntegrationPatterns:
    ...
    def test_sub_dependency_patterns(self):
        ...
        def sub_dependency(trading_service: TradingService = Depends(get_trading_service)):

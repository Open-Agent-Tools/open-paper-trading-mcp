...
class TestApplicationConfiguration:
    ...
    def test_app_basic_configuration(self):
        ...
        assert (
            app.description
            == "A FastAPI web application for paper trading with MCP support"
        )
...
class TestApplicationLifespan:
    ...
    async def test_lifespan_cache_warming_fallback(
        ...
        mock_factory = MagicMock()
        mock_factory.create_adapter.side_effect = [
            None,
            MagicMock(),
        ]  # Robinhood fails, test succeeds
        mock_factory.start_cache_warming = AsyncMock()
        mock_factory.stop_cache_warming = AsyncMock()
        mock_adapter_factory.return_value = mock_factory
    ...
    async def test_lifespan_cache_warming_failure(
        ...
        mock_print.assert_any_call(
            "Cache warming failed to start: Adapter creation failed"
        )
...
class TestMCPServerIntegration:
    ...
    def test_run_mcp_server_success(self, mock_settings, mock_mcp_instance):
        ...
        mock_mcp_instance.run.assert_called_once_with(
            host="localhost", port=2081, transport="sse"
        )
    ...
    def test_run_mcp_server_not_available(self, mock_print):
        ...
        mock_print.assert_called_once_with(
            "MCP server not available (likely in test mode)"
        )

    @patch("app.main.mcp_instance")
    @patch("app.main.settings")
    @patch("app.main.print")
    def test_run_mcp_server_exception_handling(
        self, mock_print, mock_settings, mock_mcp_instance
    ):
    ...
    @patch("os.getenv")
    def test_main_with_mcp_server(
        self, mock_getenv, mock_uvicorn_run, mock_thread_class, mock_mcp_instance
    ):
        ...
        mock_uvicorn_run.assert_called_once_with(
            "app.main:app", host="0.0.0.0", port=2080, reload=False
        )
    ...
    def test_main_development_mode(self, mock_getenv, mock_uvicorn_run):
        ...
        mock_uvicorn_run.assert_called_once_with(
            "app.main:app", host="0.0.0.0", port=2080, reload=True
        )
...
class TestExceptionHandlers:
    ...
    async def test_custom_exception_handler(self):
        ...
        exc = CustomException(detail="Test error message", status_code=400)
...
class TestApplicationIntegration:
    ...
    def test_api_routes_included(self):
        ...
        any("/api/v1" in path for path in route_paths)

...
from app.services.strategies.models import (
    AssetStrategy,
    SpreadStrategy,
    CoveredStrategy,
    SpreadType,
...
class TestMaintenanceMarginService:
    ...
    def test_calculate_long_stock_margin(self, margin_service):
        ...
        margin_req = margin_service._calculate_asset_margin(
            strategy, margin_service.quote_adapter
        )
    ...
    def test_calculate_naked_call_margin(self, margin_service):
        ...
        strategy = AssetStrategy(
            asset=asset_factory("AAPL241220C00160000"), quantity=-1
        )
        margin_req = margin_service._calculate_asset_margin(
            strategy, margin_service.quote_adapter
        )
    ...
    def test_calculate_credit_spread_margin(self, margin_service):
        ...
        strategy = SpreadStrategy(
            sell_option=sell_option, buy_option=buy_option, quantity=1
        )

        margin_req = margin_service._calculate_spread_margin(
            strategy, margin_service.quote_adapter
        )
    ...
    def test_calculate_covered_call_margin(self, margin_service):
        ...
        strategy = CoveredStrategy(
            asset=Stock(symbol="AAPL"),
            sell_option=asset_factory("AAPL241220C00160000"),
            quantity=1,
        )
        margin_req = margin_service._calculate_covered_margin(
            strategy, margin_service.quote_adapter
        )
        assert margin_req.margin_requirement == 0

...
class TestMCPOrderManagement:
    ...
    async def test_create_order_error(self, mock_mcp_trading_service):
        ...
        mock_mcp_trading_service.create_order.side_effect = Exception(
            "Insufficient funds"
        )
...
class TestMCPPortfolio:
    ...
    async def test_get_all_positions_success(self, mock_mcp_trading_service):
        ...
        mock_mcp_trading_service.get_positions.return_value = [
            mock_position1,
            mock_position2,
        ]
...
class TestMCPOptionsTools:
    ...
    async def test_create_multi_leg_order_success(self, mock_mcp_trading_service):
        ...
        mock_mcp_trading_service.create_multi_leg_order_from_request.return_value = (
            mock_order
        )

        # Act
        result = await create_multi_leg_order(
            CreateMultiLegOrderArgs(legs=legs, order_type=order_type)
        )
        result_dict = json.loads(result)
    ...
    async def test_calculate_option_greeks_success(self, mock_mcp_trading_service):
        ...
        with patch("app.mcp.tools.asset_factory", return_value=mock_option):
            ...
            result = await calculate_option_greeks(
                CalculateGreeksArgs(
                    option_symbol=option_symbol, underlying_price=underlying_price
                )
            )
            result_dict = json.loads(result)
    ...
    async def test_get_strategy_analysis_success(self, mock_mcp_trading_service):
        ...
        mock_mcp_trading_service.analyze_portfolio_strategies.return_value = (
            mock_analysis
        )
    ...
    async def test_simulate_option_expiration_success(self, mock_mcp_trading_service):
        ...
        result = await simulate_option_expiration(
            SimulateExpirationArgs(processing_date=processing_date, dry_run=dry_run)
        )
        result_dict = json.loads(result)

        # Assert
        mock_mcp_trading_service.simulate_expiration.assert_called_once_with(
            processing_date=processing_date, dry_run=dry_run
        )
    ...
    async def test_get_option_market_data_success(self, mock_mcp_trading_service):
        ...
        # Assert
        mock_mcp_trading_service.get_option_market_data.assert_called_once_with(
            option_id
        )

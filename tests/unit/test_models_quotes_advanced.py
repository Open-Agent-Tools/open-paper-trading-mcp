...
class TestQuoteFactory:
    ...
    def test_quote_factory_stock_quote(self):
        ...
        result = quote_factory(
            quote_date=quote_date, asset="AAPL", price=150.0, bid=149.5, ask=150.5
        )
    ...
    def test_quote_factory_with_asset_object(self):
        ...
        result = quote_factory(quote_date=quote_date, asset=stock, price=2800.0)
    ...
    def test_quote_factory_date_normalization(self):
        ...
        result1 = quote_factory(
            quote_date="2024-03-15T10:30:00Z", asset="AAPL", price=150.0
        )
        assert isinstance(result1.quote_date, datetime)

        # Date object
        date_obj = date(2024, 3, 15)
        result2 = quote_factory(quote_date=date_obj, asset="AAPL", price=150.0)
    ...
    def test_quote_factory_invalid_asset(self):
        ...
        with pytest.raises(ValueError, match="Could not create asset"):
            quote_factory(quote_date=datetime.now(), asset=None, price=100.0)
...
class TestQuote:
    ...
    def test_quote_asset_normalization(self):
        ...
        quote = Quote(asset="GOOGL", quote_date=datetime.now(), price=2800.0)
    ...
    def test_quote_asset_normalization_invalid(self):
        ...
        with pytest.raises(ValueError, match="Could not create asset"):
            Quote(asset="", quote_date=datetime.now(), price=100.0)

    def test_quote_date_normalization(self):
        """Test quote date normalization."""
        # String date
        quote1 = Quote(asset="AAPL", quote_date="2024-03-15T10:30:00Z", price=150.0)
        assert isinstance(quote1.quote_date, datetime)

        # Date object
        date_obj = date(2024, 3, 15)
        quote2 = Quote(asset="AAPL", quote_date=date_obj, price=150.0)
    ...
    def test_quote_properties(self):
        ...
        asset = Stock("TSLA")
        quote = Quote(
            asset=asset, quote_date=datetime.now(), bid=800.0, ask=801.0, price=800.5
        )
    ...
    def test_quote_is_priceable(self):
        ...
        # Zero price
        quote2 = Quote(asset="AAPL", quote_date=datetime.now(), price=0.0)
        assert quote2.is_priceable() is False

        # None price
        quote3 = Quote(asset="AAPL", quote_date=datetime.now(), price=None)
    ...
    def test_quote_validation_constraints(self):
        ...
        with pytest.raises(ValueError):
            Quote(asset="AAPL", quote_date=datetime.now(), volume=-100)
    ...
    def test_quote_edge_cases(self):
        ...
        quote = Quote(asset="PENNY", quote_date=datetime.now(), bid=0.0, ask=0.0)
...
class TestOptionQuote:
    ...
    def test_option_quote_validation_non_option_asset(self):
        ...
        with pytest.raises(ValueError, match="OptionQuote requires an Option asset"):
            OptionQuote(asset=stock, quote_date=datetime.now(), price=150.0)
    ...
    def test_option_quote_properties(self):
        ...
        quote = OptionQuote(
            asset=option, quote_date=quote_date, price=25.0, underlying_price=950.0
        )
    ...
    def test_option_quote_has_greeks(self):
        ...
        # Without IV
        quote1 = OptionQuote(asset=option, quote_date=datetime.now(), price=5.0)
        assert quote1.has_greeks() is False

        # With IV
        quote2 = OptionQuote(asset=option, quote_date=datetime.now(), price=5.0, iv=0.3)
        assert quote2.has_greeks() is True

    def test_option_quote_intrinsic_value(self):
        """Test intrinsic value calculation."""
        call_option = Call(
            underlying="AAPL", strike=150.0, expiration_date=date.today()
        )
...
class TestQuoteResponse:
    ...
    def test_quote_response_creation(self):
        ...
        response = QuoteResponse(
            quote=quote, data_source="test_adapter", cached=True, cache_age_seconds=300
        )
    ...
    def test_quote_response_with_option_quote(self):
        ...
        response = QuoteResponse(
            quote=option_quote, data_source="robinhood", cached=False
        )
...
class TestOptionsChain:
    ...
    def test_options_chain_get_strikes(self):
        ...
        chain = OptionsChain(
            underlying_symbol="SPY", expiration_date=exp_date, calls=calls, puts=puts
        )
    ...
    def test_options_chain_get_options_by_strike(self):
        ...
        chain = OptionsChain(
            underlying_symbol="QQQ", expiration_date=exp_date, calls=[call], puts=[put]
        )
    ...
    def test_options_chain_filter_by_strike_range(self):
        ...
        for option_class, strike, price in options_data:
            quote = OptionQuote(
                asset=option_class(
                    underlying="AAPL", strike=strike, expiration_date=exp_date
                ),
                quote_date=datetime.now(),
                price=price,
            )
        ...
        chain = OptionsChain(
            underlying_symbol="AAPL", expiration_date=exp_date, calls=calls, puts=puts
        )
    ...
    def test_options_chain_get_option_by_delta(self):
        ...
        chain = OptionsChain(
            underlying_symbol="AAPL", expiration_date=exp_date, calls=calls
        )
    ...
    def test_options_chain_empty(self):
        ...
        chain = OptionsChain(underlying_symbol="TEST", expiration_date=date.today())
...
class TestQuoteEdgeCases:
    ...
    def test_quote_with_inverted_spread(self):
        ...
        quote = Quote(asset="WEIRD", quote_date=datetime.now(), bid=101.0, ask=100.0)
    ...
    def test_quote_date_edge_cases(self):
        ...
        quote1 = Quote(
            asset="AAPL", quote_date="2024-03-15T10:30:00+05:00", price=150.0
        )
        assert isinstance(quote1.quote_date, datetime)

        # Minimal date object
        minimal_date = date(2024, 1, 1)
        quote2 = Quote(asset="AAPL", quote_date=minimal_date, price=150.0)

...
from app.services.order_impact import OrderImpactService, OrderImpactAnalysis, AccountSnapshot
...
class TestOrderImpactService:
    ...
    def test_create_account_snapshot(self):
        ...
        snapshot = self.service._create_account_snapshot(self.mock_account, self.mock_quote_adapter)
    ...
    def test_simulate_order_execution(self):
        ...
        aapl_position = next((p for p in positions if p.get('symbol', p.symbol) == 'AAPL'), None)
    ...
    def test_position_changes_analysis(self):
        ...
        after_positions = [
            ...
            Position(symbol="GOOGL", quantity=5, avg_price=2800.0, current_price=2850.0)
        ]
        
        changes = self.service._analyze_position_changes(before_positions, after_positions)
    ...
    def test_greeks_impact_calculation(self):
        ...
        greeks_impact = self.service._calculate_greeks_impact(before_positions, after_positions)
    ...
    def test_risk_assessment(self):
        ...
        risky_account = {
            'cash_balance': 1000.0,  # Low cash
            'positions': []
        }
        
        before_snapshot = self.service._create_account_snapshot(risky_account, self.mock_quote_adapter)
        ...
        after_snapshot = self.service._create_account_snapshot(after_account, self.mock_quote_adapter)
        
        warnings = self.service._assess_risks(before_snapshot, after_snapshot, large_order)
    ...
    def test_options_risk_assessment(self):
        ...
        before_snapshot = self.service._create_account_snapshot(self.mock_account, self.mock_quote_adapter)
        after_account = self.service._simulate_order_execution(
            self.mock_account,
            option_order,
            self.mock_quote_adapter
        ...
        warnings = self.service._assess_risks(before_snapshot, after_snapshot, option_order)
    ...
    def test_validation_integration(self):
        ...
        errors = self.service._validate_order(self.mock_account, order, self.mock_quote_adapter)
    ...
    def test_estimated_fill_prices(self):
        ...
        estimated_prices = {"AAPL": 158.0}  # Different from order price
    ...
    def test_order_type_descriptions(self):
        ...
        # Single order
        single_order = Order(symbol="AAPL", order_type=OrderType.BUY, quantity=5, price=160.0)

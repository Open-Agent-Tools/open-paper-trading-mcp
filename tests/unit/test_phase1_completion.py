...
from app.models.trading import (
    Order, MultiLegOrder, OrderLeg, OrderType, OrderCondition, OrderStatus,
    Position, OrderCreate, MultiLegOrderCreate, OrderLegCreate
)
from app.models.quotes import Quote, OptionQuote, OptionsChain, quote_factory
...
from app.services.strategies import StrategyRecognitionService, group_into_basic_strategies
from app.services.margin import MaintenanceMarginService, calculate_maintenance_margin
from app.services.order_impact import OrderImpactService, analyze_order_impact
...
class TestIntegration:
    ...
    def test_end_to_end_option_workflow(self):
        ...
        margin_result = margin_service.calculate_maintenance_margin(positions=[position])
...
class TestAPIIntegration:
    ...
    def test_quote_endpoint(self):
        ...
        import requests
        try:
            response = requests.get("http://localhost:2080/api/v1/trading/quote/AAPL", timeout=5)
            assert response.status_code == 200
            data = response.json()
    ...
    def test_order_creation_endpoint(self):
        ...
        try:
            ...
            response = requests.post(
                "http://localhost:2080/api/v1/trading/order",
                json=order_data,
                timeout=5
            )
            assert response.status_code == 200

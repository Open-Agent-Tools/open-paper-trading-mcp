...
class TestAccountSchema:
    ...
    def test_account_creation_minimal(self):
        ...
        account = Account(id="test_123", cash_balance=1000.0)
    ...
    def test_cash_property_alias(self):
        ...
        account = Account(id="test_789", cash_balance=2500.0)
...
class TestAccountValidationMixin:
    ...
    def test_valid_cash_balance(self):
        ...
        account = Account(id="cash_test", cash_balance=1000.0)
        assert account.cash_balance == 1000.0

    def test_zero_cash_balance(self):
        """Test zero cash balance is allowed."""
        account = Account(id="zero_cash_test", cash_balance=0.0)
    ...
    def test_negative_cash_balance_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Account(id="negative_test", cash_balance=-1000.0)
    ...
    def test_valid_owner_validation(self):
        ...
        account = Account(id="owner_test", cash_balance=1000.0, owner="Valid Owner")
        assert account.owner == "Valid Owner"

    def test_empty_owner_validation(self):
        """Test empty owner string validation."""
        with pytest.raises(ValidationError) as exc_info:
            Account(id="empty_owner_test", cash_balance=1000.0, owner="")
    ...
    def test_whitespace_only_owner_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Account(id="whitespace_owner_test", cash_balance=1000.0, owner="   ")
    ...
    def test_owner_trimming(self):
        ...
        account = Account(
            id="trim_owner_test", cash_balance=1000.0, owner="  Trimmed Owner  "
        )
        assert account.owner == "Trimmed Owner"

    def test_valid_name_validation(self):
        """Test valid name field validation."""
        account = Account(id="name_test", cash_balance=1000.0, name="Valid Name")
        assert account.name == "Valid Name"

    def test_empty_name_validation(self):
        """Test empty name string validation."""
        with pytest.raises(ValidationError) as exc_info:
            Account(id="empty_name_test", cash_balance=1000.0, name="")
    ...
    def test_whitespace_only_name_validation(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            Account(id="whitespace_name_test", cash_balance=1000.0, name="   ")
    ...
    def test_name_trimming(self):
        ...
        account = Account(
            id="trim_name_test", cash_balance=1000.0, name="  Trimmed Name  "
        )
    ...
    def test_none_owner_and_name_allowed(self):
        ...
        account = Account(id="none_test", cash_balance=1000.0, owner=None, name=None)
        assert account.owner is None
        assert account.name is None
...
class TestAccountPositionsIntegration:
    ...
    def test_account_with_stock_positions(self):
        ...
        positions = [
            ...
            Position(
                symbol="GOOGL", quantity=50, avg_price=2800.0, current_price=2850.0
            ),
        ]

        account = Account(
            id="stock_positions_test", cash_balance=5000.0, positions=positions
        )
    ...
    def test_account_with_mixed_positions(self):
        ...
        stock_position = Position(symbol="AAPL", quantity=100, avg_price=150.0)
    ...
    def test_account_empty_positions_default(self):
        ...
        account = Account(id="empty_positions_test", cash_balance=1000.0)
...
class TestAccountEdgeCases:
    ...
    def test_very_large_cash_balance(self):
        ...
        account = Account(id="large_balance_test", cash_balance=large_balance)
        assert account.cash_balance == large_balance

    def test_very_long_id(self):
        """Test account with very long ID."""
        long_id = "a" * 1000
        account = Account(id=long_id, cash_balance=1000.0)
    ...
    def test_unicode_in_fields(self):
        ...
        account = Account(
            id="unicode_测试_123", cash_balance=1000.0, name="测试账户", owner="用户名"
        )
...
class TestAccountJSONSerialization:
    ...
    def test_json_serialization_roundtrip(self):
        ...
        original_account = Account(
            id="json_test", cash_balance=5000.0, name="JSON Test", owner="JSON Owner"
        )
    ...
    def test_json_with_positions_roundtrip(self):
        ...
        original_account = Account(
            id="json_positions_test", cash_balance=3000.0, positions=[position]
        )

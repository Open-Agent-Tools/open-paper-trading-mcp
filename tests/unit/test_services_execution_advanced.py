...
class TestOrderExecutionEngine:
    ...
    async def test_execute_market_buy_order(self, order_execution_engine, sample_portfolio):
    ...
    @pytest_asyncio.async_test
    async def test_execute_market_sell_order(self, order_execution_engine, sample_portfolio):
    ...
    @pytest_asyncio.async_test
    async def test_insufficient_shares_error(self, order_execution_engine, sample_portfolio):
    ...
    @pytest_asyncio.async_test
    async def test_insufficient_cash_error(self, order_execution_engine, sample_portfolio):
    ...
    @pytest_asyncio.async_test
    async def test_new_position_creation(self, order_execution_engine, sample_portfolio):
...
class TestExecutionStrategies:
    ...
    @pytest_asyncio.async_test
    async def test_twap_execution_strategy(self, order_execution_engine, sample_portfolio):
        ...
        with patch.object(order_execution_engine, '_execute_twap_strategy') as mock_twap:
            ...
            result = await order_execution_engine.execute_order(large_order, sample_portfolio)
    ...
    @pytest_asyncio.async_test
    async def test_vwap_execution_strategy(self, order_execution_engine, sample_portfolio):
        ...
        with patch.object(order_execution_engine, '_execute_vwap_strategy') as mock_vwap:
    ...
    @pytest_asyncio.async_test
    async def test_iceberg_execution_strategy(self, order_execution_engine, sample_portfolio):
        ...
        with patch.object(order_execution_engine, '_execute_iceberg_strategy') as mock_iceberg:
            ...
            result = await order_execution_engine.execute_order(iceberg_order, sample_portfolio)
...
class TestMarketImpactCalculation:
    ...
    def test_square_root_impact_model(self):
        ...
        impacts = [calculator.calculate_sqrt_impact(size, quote) for size in order_sizes]
    ...
    def test_volume_based_impact(self):
        ...
        high_vol_impact = calculator.calculate_volume_impact(order_quantity, high_volume_quote)
        low_vol_impact = calculator.calculate_volume_impact(order_quantity, low_volume_quote)
...
class TestSlippageEstimation:
    ...
    def test_volatility_based_slippage(self):
        ...
        with patch.object(estimator, '_get_historical_volatility') as mock_volatility:
            ...
            high_vol_slippage = estimator.estimate_volatility_slippage(1000, quote, 15)  # 15 min execution
...
class TestExecutionQueue:
    ...
    async def test_queue_order_processing(self):
        ...
        orders = [
            OrderCreate(symbol="AAPL", quantity=100, order_type=OrderType.MARKET, side="buy"),
            OrderCreate(symbol="GOOGL", quantity=10, order_type=OrderType.LIMIT, side="buy", limit_price=2500.0),
            OrderCreate(symbol="MSFT", quantity=50, order_type=OrderType.MARKET, side="sell")
        ]
    ...
    async def test_priority_queue_ordering(self):
        ...
        low_priority_order = OrderCreate(symbol="LOW", quantity=100, order_type=OrderType.MARKET, side="buy")
        high_priority_order = OrderCreate(symbol="HIGH", quantity=100, order_type=OrderType.MARKET, side="buy")
        medium_priority_order = OrderCreate(symbol="MED", quantity=100, order_type=OrderType.MARKET, side="buy")
    ...
    async def test_queue_cancellation(self):
        ...
        order = OrderCreate(symbol="AAPL", quantity=100, order_type=OrderType.LIMIT, side="buy", limit_price=150.0)
...
class TestOrderStateMachine:
    ...
    def test_order_state_transitions(self):
        ...
        # Valid transitions
        assert state_machine.transition(order, OrderStatus.PENDING, OrderStatus.SUBMITTED) is True
        assert state_machine.transition(order, OrderStatus.SUBMITTED, OrderStatus.PARTIALLY_FILLED) is True
        assert state_machine.transition(order, OrderStatus.PARTIALLY_FILLED, OrderStatus.FILLED) is True
    ...
    def test_invalid_state_transitions(self):
        ...
        order = OrderCreate(symbol="AAPL", quantity=100, order_type=OrderType.MARKET, side="buy")
        
        # Invalid transitions should be rejected
        assert state_machine.transition(order, OrderStatus.PENDING, OrderStatus.FILLED) is False  # Skip states
        assert state_machine.transition(order, OrderStatus.CANCELLED, OrderStatus.SUBMITTED) is False  # From terminal state
        assert state_machine.transition(order, OrderStatus.FILLED, OrderStatus.PENDING) is False  # Backwards
    ...
    def test_cancellation_transitions(self):
        ...
        assert state_machine.transition(order, OrderStatus.SUBMITTED, OrderStatus.CANCELLED) is True
        
        # Reset for another test
        order2 = OrderCreate(symbol="GOOGL", quantity=10, order_type=OrderType.LIMIT, side="sell", limit_price=2600.0)
        state_machine.transition(order2, OrderStatus.PENDING, OrderStatus.SUBMITTED)
        state_machine.transition(order2, OrderStatus.SUBMITTED, OrderStatus.PARTIALLY_FILLED)
        
        # Can cancel partially filled orders
        assert state_machine.transition(order2, OrderStatus.PARTIALLY_FILLED, OrderStatus.CANCELLED) is True
...
class TestMultiLegOrderExecution:
    ...
    @pytest_asyncio.async_test
    async def test_spread_order_execution(self, order_execution_engine, sample_portfolio):
        ...
        with patch.object(order_execution_engine, '_execute_spread_order') as mock_spread:
            mock_spread.return_value = OrderExecutionResult(
                ...
                positions_created=[
                    Position(symbol="AAPL240119C00155000", quantity=1, average_cost=5.0),
                    Position(symbol="AAPL240119C00160000", quantity=-1, average_cost=-3.0)
                ]
            )
            
            result = await order_execution_engine.execute_multileg_order(spread_order, sample_portfolio)
    ...
    @pytest_asyncio.async_test
    async def test_iron_condor_execution(self, order_execution_engine, sample_portfolio):
        """Test execution of iron condor strategy."""
        iron_condor_legs = [
            OrderLeg(symbol="AAPL240119P00145000", quantity=1, side="buy", limit_price=1.0),
            OrderLeg(symbol="AAPL240119P00150000", quantity=1, side="sell", limit_price=2.0),
            OrderLeg(symbol="AAPL240119C00160000", quantity=1, side="sell", limit_price=3.0),
            OrderLeg(symbol="AAPL240119C00165000", quantity=1, side="buy", limit_price=1.5)
        ]
        ...
        with patch.object(order_execution_engine, '_execute_iron_condor') as mock_condor:
            mock_condor.return_value = OrderExecutionResult(
                ...
                positions_created=[
                    Position(symbol="AAPL240119P00145000", quantity=1, average_cost=1.0),
                    Position(symbol="AAPL240119P00150000", quantity=-1, average_cost=-2.0),
                    Position(symbol="AAPL240119C00160000", quantity=-1, average_cost=-3.0),
                    Position(symbol="AAPL240119C00165000", quantity=1, average_cost=1.5)
                ]
            )
            
            result = await order_execution_engine.execute_multileg_order(iron_condor, sample_portfolio)
...
class TestExecutionPerformanceOptimization:
    ...
    @pytest_asyncio.async_test
    async def test_batch_order_execution(self, order_execution_engine, sample_portfolio):
        """Test batch execution of multiple orders."""
        orders = [
            OrderCreate(symbol="AAPL", quantity=10, order_type=OrderType.MARKET, side="buy"),
            OrderCreate(symbol="GOOGL", quantity=1, order_type=OrderType.MARKET, side="buy"),
            OrderCreate(symbol="MSFT", quantity=5, order_type=OrderType.MARKET, side="buy")
        ]
        
        start_time = asyncio.get_event_loop().time()
        
        # Execute orders in batch
        results = await order_execution_engine.execute_batch_orders(orders, sample_portfolio)
    ...
    @pytest_asyncio.async_test
    async def test_concurrent_execution_safety(self, order_execution_engine, sample_portfolio):
        ...
        sell_results = [r for r in successful_results if r.success and r.cash_change > 0]

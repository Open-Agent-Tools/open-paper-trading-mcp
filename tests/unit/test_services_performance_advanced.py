...
def sample_metrics():
    ...
    for i in range(100):
        metrics.append(PerformanceMetric(
            ...
            end_time=base_time + i * 0.1 + 0.05 + (i % 3) * 0.01,  # Variable latency
...
class TestPerformanceMetricCollection:
    ...
    def test_record_metric_failure(self, performance_benchmarker):
        ...
        assert "Database connection timeout" in performance_benchmarker.error_trackers["failing_operation"]["errors"]
    ...
    async def test_context_manager_metric_recording(self, performance_benchmarker):
        ...
        async with performance_benchmarker.measure_operation("async_test_op") as measurement:
            # Simulate async work
...
class TestLatencyAnalysis:
    ...
    def test_latency_distribution_calculation(self, performance_benchmarker, sample_metrics):
        """Test latency distribution calculation."""
        for metric in sample_metrics:
            performance_benchmarker.record_metric(metric)
        
        distribution = performance_benchmarker.calculate_latency_distribution("test_operation_0")
    ...
    def test_latency_percentile_accuracy(self, performance_benchmarker):
        ...
        distribution = performance_benchmarker.calculate_latency_distribution("known_latency_test")
...
class TestOptimizationRecommendations:
    ...
    def test_latency_optimization_recommendations(self, performance_benchmarker, sample_metrics):
        """Test latency optimization recommendations."""
        for metric in sample_metrics:
            performance_benchmarker.record_metric(metric)
        
        recommendations = performance_benchmarker.generate_optimization_recommendations()
        
        assert isinstance(recommendations, list)
        assert len(recommendations) > 0
        
        # Should have latency-related recommendations
        latency_recs = [r for r in recommendations if "latency" in r.description.lower()]
    ...
    def test_throughput_optimization_recommendations(self, performance_benchmarker):
        ...
        throughput_recs = [r for r in recommendations if "throughput" in r.description.lower()]
        assert len(throughput_recs) > 0
        
        # Should suggest concurrency or batching improvements
        assert any("concurrency" in r.description.lower() or "batch" in r.description.lower() 
    ...
    def test_resource_optimization_recommendations(self, performance_benchmarker):
        ...
        with patch.object(performance_benchmarker, 'collect_system_metrics') as mock_metrics:
            ...
            resource_recs = [r for r in recommendations 
                           if r.category == "resources" or "cpu" in r.description.lower() 
                           or "memory" in r.description.lower()]
            assert len(resource_recs) > 0
            
            # Should suggest resource optimization
            assert any("cpu" in r.description.lower() or "memory" in r.description.lower()
...
class TestConcurrencyAndStressScenarios:
    ...
    async def test_concurrent_metric_collection(self, performance_benchmarker):
        ...
        assert len(performance_benchmarker.metrics_buffer) == 250  # 5 workers * 50 metrics
...
class TestReportingAndVisualization:
    ...
    def test_performance_report_generation(self, performance_benchmarker, sample_metrics):
    ...
    def test_comparative_analysis_report(self, performance_benchmarker):
        ...
        for op_name, base_latency in zip(operations, base_latencies):
            for i in range(30):
                metric = PerformanceMetric(
                    operation=op_name,
                    start_time=time.time() + i * 0.1,
        ...
        assert latencies[0] < latencies[1] < latencies[2]  # Should be ordered by performance
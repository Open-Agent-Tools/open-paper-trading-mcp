...
from app.schemas.orders import Order, OrderCreate, OrderStatus, OrderType, OrderCondition
...
class MockQuoteAdapter(QuoteAdapter):
    ...
    async def get_options_chain(self, symbol: str, expiration_date: str = None) -> OptionsChain:
...
async def trading_service(async_db_session):
    ...
    # Set up test quote
    mock_adapter.set_quote("AAPL", Quote(
        symbol="AAPL",
        current_price=150.0,
        bid=149.5,
        ask=150.5,
...
class TestTradingServiceInitialization:
    ...
    def test_initialization_with_default_adapter(self):
        ...
        with patch('app.adapters.config.get_adapter_factory') as mock_factory:
            ...
            mock_factory.return_value.create_adapter.return_value = mock_adapter_instance
...
class TestAsyncDatabaseOperations:
    ...
    @pytest_asyncio.async_test
    async def test_ensure_account_exists_skips_existing(self, trading_service, sample_account):
...
class TestOrderManagement:
    ...
    async def test_create_order_success(self, trading_service, sample_account):
        ...
        with patch.object(trading_service.order_execution, 'execute_order') as mock_execute:
    ...
    @pytest_asyncio.async_test
    async def test_create_order_validation_failure(self, trading_service, sample_account):
        ...
        with patch.object(trading_service.account_validation, 'validate_order') as mock_validate:
...
class TestAsyncConcurrency:
    ...
    @pytest_asyncio.async_test
    async def test_concurrent_portfolio_operations(self, trading_service, sample_account):
...
class TestErrorHandling:
    ...
    async def test_account_not_found_handling(self):
        ...
        mock_adapter = MockQuoteAdapter()
        service = TradingService(quote_adapter=mock_adapter, account_owner="nonexistent_user")

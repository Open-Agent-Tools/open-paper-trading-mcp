...
class TestAccountValidator:
    ...
    def test_validate_account_state_positive_cash(
        ...
        result = account_validator.validate_account_state(
            cash_balance=25000.0, positions=sample_portfolio.positions
        )
    ...
    def test_validate_account_state_negative_cash(
        ...
        with pytest.raises(ValidationError, match="Insufficient cash"):
            account_validator.validate_account_state(
                cash_balance=-1000.0, positions=sample_portfolio.positions
            )

    def test_validate_account_state_zero_cash(
        self, account_validator, sample_portfolio
    ):
        """Test account validation with zero cash balance."""
        result = account_validator.validate_account_state(
            cash_balance=0.0, positions=sample_portfolio.positions
        )
    ...
    def test_validate_buying_power(self, account_validator):
        ...
        with pytest.raises(ValidationError, match="Insufficient buying power"):
            account_validator.validate_buying_power(
                required_amount=50000.0, available_buying_power=30000.0
            )

        # Test sufficient buying power
        result = account_validator.validate_buying_power(
            required_amount=20000.0, available_buying_power=30000.0
        )
    ...
    def test_validate_account_restrictions(self, account_validator):
        ...
        with pytest.raises(ValidationError, match="Account is restricted"):
            account_validator.validate_account_restrictions(
                is_restricted=True, restriction_reason="Good faith violation"
            )
...
class TestOrderValidator:
    ...
    def test_validate_order_invalid_quantity(self, order_validator):
        ...
        order_zero = OrderCreate(
            symbol="AAPL", quantity=0, order_type=OrderType.MARKET, side="buy"
        )
    ...
    def test_validate_order_price_ranges(self, order_validator):
        ...
        with patch.object(order_validator, "_get_current_price") as mock_price:
            ...
            result = order_validator.validate_order(
                order_high, enable_price_collar=True
            )
            assert result.is_valid is False
            assert any(
                "price collar" in error.lower() or "far from market" in error.lower()
    ...
    def test_validate_fractional_shares(self, order_validator):
        ...
        with patch.object(
            order_validator, "_supports_fractional_shares"
        ) as mock_fractional:
...
class TestMultiLegOrderValidator:
    ...
    def test_validate_spread_order_valid(self, multileg_validator):
        ...
        multileg_order = MultiLegOrder(
            strategy_type="call_spread", legs=legs, net_debit=2.0
        )
    ...
    def test_validate_spread_legs_mismatch(self, multileg_validator):
        ...
        multileg_order = MultiLegOrder(strategy_type="call_spread", legs=legs)

        result = multileg_validator.validate_multileg_order(multileg_order)
        assert result.is_valid is False
        assert any(
            "underlying" in error.lower() or "mismatch" in error.lower()
            for error in result.errors
        )
    ...
    def test_validate_iron_condor(self, multileg_validator):
        ...
        iron_condor = MultiLegOrder(
            strategy_type="iron_condor", legs=legs, net_credit=0.5
        )
    ...
    def test_validate_unbalanced_strategy(self, multileg_validator):
        ...
        assert result.is_valid is False
        assert any(
            "balanced" in error.lower() or "ratio" in error.lower()
            for error in result.errors
        )
    ...
    def test_validate_strategy_profit_loss(self, multileg_validator):
        ...
        legs = [
            OrderLeg(
                symbol="AAPL240119C00155000", quantity=1, side="buy", limit_price=10.0
            ),  # Expensive
            OrderLeg(
                symbol="AAPL240119C00160000", quantity=1, side="sell", limit_price=2.0
            ),  # Cheap
        ]

        # This would result in guaranteed loss (paying 8.0 net debit for 5.0 max profit)
        multileg_order = MultiLegOrder(
            strategy_type="call_spread", legs=legs, net_debit=8.0
        )

        result = multileg_validator.validate_multileg_order(
            multileg_order, check_economics=True
        )
        assert result.is_valid is False
        assert any(
            "profit" in error.lower()
            or "loss" in error.lower()
            or "economic" in error.lower()
            for error in result.errors
        )
...
class TestPositionValidator:
    ...
    def test_validate_position_size_limits(self, position_validator, sample_portfolio):
        ...
        result = position_validator.validate_position_size(
            large_position, sample_portfolio, max_position_percent=0.20
        )
        assert result.is_valid is False
        assert any(
            "concentration" in error.lower() or "limit" in error.lower()
            for error in result.errors
        )
    ...
    def test_validate_correlation_risk(self, position_validator, sample_portfolio):
        ...
        with patch.object(position_validator, "_get_correlation") as mock_correlation:
            ...
            result = position_validator.validate_correlation_risk(
                correlated_position, sample_portfolio, max_correlation=0.80
            )
    ...
    def test_validate_options_position_risk(self, position_validator):
        ...
        assert result.is_valid is False
        assert any(
            "naked" in error.lower() or "option" in error.lower()
            for error in result.errors
        )
...
class TestBusinessRuleValidator:
    ...
    def test_validate_options_approval_level(self, business_rule_validator):
        ...
        assert result.is_valid is False
        assert any(
            "options level" in error.lower() or "approval" in error.lower()
            for error in result.errors
        )
    ...
    def test_validate_wash_sale_rules(self, business_rule_validator):
        ...
        with patch.object(
            business_rule_validator, "_get_recent_transactions"
            ...
            buy_order = OrderCreate(
                symbol="AAPL", quantity=50, order_type=OrderType.MARKET, side="buy"
            )
    ...
    def test_validate_good_faith_violations(self, business_rule_validator):
        ...
        assert result.is_valid is False
        assert any(
            "good faith" in error.lower() or "unsettled" in error.lower()
            for error in result.errors
        )
    ...
    def test_validate_free_riding_rules(self, business_rule_validator):
        ...
        with patch.object(
            business_rule_validator, "_get_pending_settlements"
        ) as mock_settlements:
            ...
            sell_order = OrderCreate(
                symbol="GOOGL", quantity=10, order_type=OrderType.MARKET, side="sell"
            )
            ...
            assert result.is_valid is False
            assert any(
                "free riding" in error.lower() or "settlement" in error.lower()
                for error in result.errors
            )
...
class TestAsyncValidation:
    ...
    async def test_async_order_validation(self, order_validator):
        ...
        with patch.object(
            order_validator, "_get_market_data_async"
        ) as mock_market_data:
    ...
    async def test_async_portfolio_validation(
        ...
        with patch.object(
            account_validator, "_calculate_risk_metrics_async"
        ) as mock_risk:
            ...
            result = await account_validator.validate_portfolio_async(
                sample_portfolio, include_risk_analysis=True
            )
    ...
    async def test_concurrent_validation(self, order_validator):
        ...
        orders = [
            OrderCreate(
                symbol=f"STOCK{i}", quantity=10, order_type=OrderType.MARKET, side="buy"
            )
            for i in range(5)
        ]
        ...
        validation_tasks = [
            order_validator.validate_order_async(order) for order in orders
        ]
    ...
    async def test_validation_timeout_handling(self, order_validator):
        ...
        with patch.object(
            ...
            mock_market_data.side_effect = TimeoutError("Market data timeout")

            result = await order_validator.validate_order_async(
                order, check_market_data=True, timeout_seconds=1.0
            )
...
class TestValidationRuleEngine:
    ...
    def test_custom_validation_rule(self, business_rule_validator):
        ...
        business_rule_validator.add_custom_rule(
            "large_order_check", custom_large_order_rule
        )
    ...
    def test_conditional_validation_rules(self, business_rule_validator):
        ...
        large_order = OrderCreate(
            symbol="AAPL", quantity=1000, order_type=OrderType.MARKET, side="buy"
        )
    ...
    def test_rule_priority_and_ordering(self, business_rule_validator):
        ...
        business_rule_validator.add_custom_rule(
            "medium", medium_priority_rule, priority=2
        )
    ...
    async def test_validation_performance_benchmark(self, order_validator):
        ...
        for i in range(0, len(orders), batch_size):
            ...
            batch_results = await asyncio.gather(
                *[order_validator.validate_order_async(order) for order in batch]
            )

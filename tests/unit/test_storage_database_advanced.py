...
class TestDatabaseEngineCreation:
    ...
    def test_engine_creation_with_postgresql_url(self, mock_async_engine, mock_sync_engine):
        ...
        mock_async_engine.assert_called_once_with("postgresql+asyncpg://user:pass@host:5432/db")

    @patch.dict(os.environ, {"DATABASE_URL": "postgresql+asyncpg://user:pass@host:5432/db"})
    @patch("app.storage.database.create_engine")
    @patch("app.storage.database.create_async_engine")
    def test_engine_creation_with_asyncpg_url(self, mock_async_engine, mock_sync_engine):
    ...
    @patch("app.storage.database.create_engine")
    @patch("app.storage.database.create_async_engine")
    def test_testing_environment_uses_test_database(self, mock_async_engine, mock_sync_engine):
        """Test that testing environment uses TEST_DATABASE_URL."""
        import importlib
        import app.storage.database
        importlib.reload(app.storage.database)
        
        mock_sync_engine.assert_called_once_with("postgresql://test:pass@testhost:5432/testdb")
        mock_async_engine.assert_called_once_with("postgresql+asyncpg://test:pass@testhost:5432/testdb")
    ...
    def test_session_factory_creation(self):
        ...
        from app.storage.database import SessionLocal, AsyncSessionLocal
...
class TestDatabaseInitialization:
    ...
    async def test_init_db_handles_metadata_errors(self, mock_engine):
        ...
        mock_conn = AsyncMock()
        mock_conn.run_sync = AsyncMock(side_effect=DatabaseError("Table creation failed", None, None))
        mock_engine.begin = AsyncMock()
        mock_engine.begin.return_value.__aenter__ = AsyncMock(return_value=mock_conn)
        mock_engine.begin.return_value.__aexit__ = AsyncMock(return_value=None)
...
class TestTransactionManagement:
    ...
    def test_sync_transaction_rollback_on_error(self, mock_session_factory):
        ...
        mock_session = MagicMock(spec=Session)
        mock_session.execute.side_effect = [None, DatabaseError("Constraint violation", None, None)]
        mock_session_factory.return_value = mock_session
        ...
        with pytest.raises(DatabaseError):
            with get_sync_session() as session:
                session.execute(text("INSERT INTO accounts (id) VALUES (1)"))
                session.execute(text("INSERT INTO orders (account_id) VALUES (999)"))  # This fails
        ...
        async for session in get_async_session():
            ...
            async with session.begin():
                ...
                async with session.begin():  # Nested/savepoint
                    await session.execute(text("INSERT INTO orders (account_id) VALUES (1)"))
            break
...
class TestEnvironmentConfiguration:
    ...
    def test_database_url_from_settings(self, mock_settings):
        ...
        mock_settings.DATABASE_URL = "postgresql://settings:pass@settings:5432/settings_db"
        
        # Clear any cached modules and reload
        import sys
        modules_to_reload = [k for k in sys.modules.keys() if k.startswith('app.storage')]
    ...
    def test_sync_and_async_urls_consistency(self):
        ...
        from app.storage.database import SYNC_DATABASE_URL, ASYNC_DATABASE_URL
        
        # Async URL should be sync URL with asyncpg driver
        if "postgresql://" in SYNC_DATABASE_URL:
            expected_async_url = SYNC_DATABASE_URL.replace("postgresql://", "postgresql+asyncpg://")
            assert ASYNC_DATABASE_URL == expected_async_url
    ...
    def test_engine_instances_are_singletons(self):
        ...
        from app.storage.database import sync_engine, async_engine
        
        # Import again to verify same instances
        from app.storage.database import sync_engine as sync_engine2, async_engine as async_engine2
...
class TestErrorHandlingAndResilience:
    ...
    def test_async_engine_creation_failure(self, mock_create_async_engine):
        ...
        mock_create_async_engine.side_effect = SQLAlchemyError("Async engine creation failed")
    ...
    async def test_async_session_creation_failure(self, mock_session_factory):
        ...
        mock_session_factory.side_effect = SQLAlchemyError("Async session creation failed")
    ...
    async def test_init_db_connection_timeout(self, mock_engine):
        ...
        mock_engine.begin.side_effect = TimeoutError("Connection timeout")

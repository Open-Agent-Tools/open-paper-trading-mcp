...
class TestConnectionPooling:
    ...
    async def test_connection_pool_exhaustion_handling(self):
        ...
        try:
            ...
            for i in range(10):  # Attempt to exceed typical pool size
                try:
...
class TestPerformanceCharacteristics:
    ...
    async def test_session_creation_performance(self):
        ...
        assert avg_time_per_session < 0.1, f"Session creation too slow: {avg_time_per_session}s"
    ...
    async def test_concurrent_session_performance(self):
        ...
        async def session_task(task_id: int):
            ...
            async for session in get_async_session():
                ...
                for i in range(3):
                    result = await session.execute(text("SELECT :val").bindparam(val=task_id + i))
    ...
    async def test_transaction_performance(self):
        ...
        assert transaction_time < 2.0, f"Batch transaction too slow: {transaction_time}s"
...
class TestConcurrencyAndThreadSafety:
    ...
    async def test_async_session_thread_safety(self):
        ...
        async def concurrent_session_task(task_id: int):
            ...
            async for session in get_async_session():
                ...
                result1 = await session.execute(text("SELECT :id").bindparam(id=task_id))
                value1 = result1.scalar()
                
                # Small delay to increase chance of concurrency issues
                await asyncio.sleep(0.001)
                
                result2 = await session.execute(text("SELECT :id").bindparam(id=task_id * 2))
    ...
    async def test_mixed_sync_async_operations(self):
        ...
        from app.storage.database import get_sync_session, get_async_session
        import threading
        import queue
...
class TestResourceManagement:
    ...
    async def test_connection_leak_prevention(self):
        ...
        initial_pool_info = None
        if hasattr(async_engine.pool, 'status'):
            initial_pool_info = async_engine.pool.status
        ...
        final_pool_info = None
        if hasattr(async_engine.pool, 'status'):
            final_pool_info = async_engine.pool.status
